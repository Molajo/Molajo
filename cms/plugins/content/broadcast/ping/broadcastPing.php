...
class BroadcastPing
{
    ...
    function pingProcess($sourceTitle, $sourceURL, $sourceSiteName)
    {
        ...
        if ($tamkaPingCounter == 0) {
            $message = MolajoTextHelper::_('No Ping Services were activated in the Tamka Library System Plugin.');
            $mainframe->enqueueMessage(MolajoTextHelper::_('Warning: ') . $message);
            return false;
        }
    }
    ...
    function ping($sourceTitle, $sourceURL, $sourceSiteName, $pingServer, $pingPort, $pingPath, $pingMethod, $pingExtended)
    {
        ...
        if ($response->faultCode() == 0) {
            /* Table */
            $message = MolajoTextHelper::_('Ping to: ') . $pingServer . " for " . $sourceTitle . " URL: " . $sourceURL . " Site name: " . $sourceSiteName;
            $mainframe->enqueueMessage(MolajoTextHelper::_('Success: ') . $message);
            return true;
        } else {
            $message = MolajoTextHelper::_('Error: ') . $pingServer . ": " . $response->faultCode() . " " . $response->faultString();
            $mainframe->enqueueMessage(MolajoTextHelper::_('Ping error: ') . $message);
            return false;
        }
