...
class plgContentJoomla extends MolajoApplicationPlugin
{
	...
	public function onContentAfterSave($context, &$article, $isNew)
	{
		...
		$default_language = MolajoApplicationComponent::getParameters('languages')->get('administrator');
	...
	public function onContentBeforeDelete($context, $data)
	{
		...
		if (isset($tableInfo[$extension]))
		{
			...
			else
			{
				...
				if ($count > 0 ) {
					$msg = MolajoText::sprintf('CATEGORIES_DELETE_NOT_ALLOWED', $data->get('title')) .
					MolajoText::plural('CATEGORIES_N_ITEMS_ASSIGNED', $count);
				...
				if (!$data->isLeaf()) {
					...
					{
						$msg = MolajoText::sprintf('CATEGORIES_DELETE_NOT_ALLOWED', $data->get('title')) .
						MolajoText::plural('CATEGORIES_HAS_SUBCATEGORY_ITEMS', $count);
						JError::raiseWarning(403, $msg);
						$result = false;
					}
				}
