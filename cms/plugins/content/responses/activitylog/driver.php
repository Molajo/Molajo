...
class ResponsesActivityLog extends MolajoPlugin
{
    function onAfterDisplayContent(&$article, &$parameters, $limitstart)
    {
         ...
         *     Parameters
         */
        $plugin =& MolajoPlugin::getPlugin('content', 'tamka_comments');
        ...
        if ($result) {
            ...
            $commentsExpiredMessage = $pluginParameters->def('closedmessage', MolajoTextHelper::_('Comments are now closed.'));
            $closeComments = true;
        }
        ...
        $module = JModuleHelper::getModule('tamka_comments_post');
        $output = '';
