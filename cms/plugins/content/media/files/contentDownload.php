...
class plgMolajoContentLinktype extends MolajoPlugin
{
    ...
    function onAfterDisplayContent(&$article, &$parameters, $limitstart)
    {
        ...
        $routerPlugin =& MolajoPlugin::getPlugin('system', 'tamka_router');
        ...
        $query = 'SELECT tag, alias FROM ' . $db->nameQuote('#__tamka_tags') .
                 ' WHERE ' . $db->nameQuote('component') . ' = "articles" ' .
        ...
        if (count($rows)) {
            $formatOutput = '<div class="tamka_article_tags"><p>' . MolajoTextHelper::_(' Tags: ');
            $comma = ' ';
        ...
        foreach ($postTags as $tag) {
            ...
            if (trim(str_replace('-', '', $tag)) == '') {
                ...
                $query = 'INSERT INTO #__tamka_tags										' .
                         ...
                         ' SELECT "articles", ' . $article->id . ', "' . $tag . '", "' . $tagAlias . '", "post"';
