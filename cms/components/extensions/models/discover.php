...
class InstallerModelDiscover extends InstallerModel
{
	...
	function discover_install()
	{
		...
		if (is_array($eid) || $eid) {
			...
			foreach($eid as $id) {
				...
				if (!$result) {
					$failed = true;
					$app->enqueueMessage(MolajoText::_('INSTALLER_MSG_DISCOVER_INSTALLFAILED').': '. $id);
				}
			}
			...
			if (!$failed) {
				$app->enqueueMessage(MolajoText::_('INSTALLER_MSG_DISCOVER_INSTALLSUCCESSFUL'));
			}
		} else {
			$app->enqueueMessage(MolajoText::_('INSTALLER_MSG_DISCOVER_NOEXTENSIONSELECTED'));
		}
	}
	...
	function purge()
	{
		...
		if ($db->Query()) {
			$this->_message = MolajoText::_('INSTALLER_MSG_DISCOVER_PURGEDDISCOVEREDEXTENSIONS');
			return true;
		} else {
			$this->_message = MolajoText::_('INSTALLER_MSG_DISCOVER_FAILEDTOPURGEEXTENSIONS');
			return false;
		}
	}
}
