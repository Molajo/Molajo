...
class InstallerModelUpdate extends JModelList
{
	...
	public function purge()
	{
		...
		if ($db->Query()) {
			$this->_message = MolajoText::_('INSTALLER_PURGED_UPDATES');
			return true;
		} else {
			$this->_message = MolajoText::_('INSTALLER_FAILED_TO_PURGE_UPDATES');
			return false;
		}
	}
	...
	public function enableSites()
	{
		...
		if ($db->Query()) {
			if ($rows = $db->getAffectedRows()) {
				$this->_message .= MolajoText::plural('INSTALLER_ENABLED_UPDATES', $rows);
			}
			return true;
		} else {
			$this->_message .= MolajoText::_('INSTALLER_FAILED_TO_ENABLE_UPDATES');
			return false;
		}
	}
	...
	private function install($update)
	{
		...
		if (isset($update->get('downloadurl')->_data)) {
			...
			MolajoError::raiseWarning('', MolajoText::_('INSTALLER_INVALID_EXTENSION_UPDATE'));
			return false;
		}
		...
		if (!$p_file) {
			MolajoError::raiseWarning('', MolajoText::sprintf('INSTALLER_PACKAGE_DOWNLOAD_FAILED', $url));
			return false;
		}
		...
		if (!$installer->update($package['dir'])) {
			...
			$msg = MolajoText::sprintf('INSTALLER_MSG_UPDATE_ERROR', MolajoText::_('INSTALLER_TYPE_TYPE_'.strtoupper($package['type'])));
			$result = false;
		} else {
			// Package updated successfully
			$msg = MolajoText::sprintf('INSTALLER_MSG_UPDATE_SUCCESS', MolajoText::_('INSTALLER_TYPE_TYPE_'.strtoupper($package['type'])));
			$result = true;
		}
