...
class InstallerModel extends JModelList
{
    ...
    protected function _getList($query, $limitstart = 0, $limit = 0)
    {
        ...
        if ($ordering == 'name' || (!empty($search) && stripos($search, 'id:') !== 0)) {
            ...
            JArrayHelper::sortObjects($result, $this->getState('list.ordering'), $this->getState('list.direction') == 'desc'
                                                     ? -1 : 1, true, $lang->getLocale());
            $total = count($result);
    ...
    private function translate(&$items)
    {
        ...
        foreach ($items as &$item) {
            ...
            $item->application = $item->application_id ? MolajoTextHelper::_('JADMINISTRATOR')
                    : MolajoTextHelper::_('JSITE');
            ...
            $item->name = MolajoTextHelper::_($item->name);
            $item->description = MolajoTextHelper::_(@$item->description);
        }
    }
}
