...
(function(a, b) {
    ...
    f.event = {add:function(a, c, d, e) {
        ...
        return a
    },guid:100000000,proxy:f.proxy,special:{ready:{setup:f.bindReady,teardown:f.noop},live:{add:function(a) {
...
new function(a) {
    a.fn.placeholder = function(b) {
        b = b || {};
        var j = b.dataKey || "placeholderValue";
        var f = b.attr || "placeholder";
        var h = b.className || "placeholder";
        var k = b.values || [];
        var c = b.blockSubmit || false;
        var e = b.blankSubmit || false;
        var g = b.onSubmit || false;
        var i = b.value || "";
        var d = b.cursor_position || 0;
        return this.filter(":input").each(
            function(l) {
                a.data(this, j, k[l] || a(this).attr(f))
            }).each(
            function() {
                if (a.trim(a(this).val()) === "") {
                    a(this).addClass(h).val(a.data(this, j))
                }
            }).focus(
            function() {
                if (a.trim(a(this).val()) === a.data(this, j)) {
                    a(this).removeClass(h).val(i)
                }
                if (a.fn.setCursorPosition) {
                    a(this).setCursorPosition(d)
                }
            }).blur(
            function() {
                if (a.trim(a(this).val()) === i) {
                    a(this).addClass(h).val(a.data(this, j))
                }
            }).each(function(l, m) {
            if (c) {
                new function(n) {
                    a(n.form).submit(function() {
                        return a.trim(a(n).val()) != a.data(n, j)
                    })
                }(m)
            } else {
                if (e) {
                    new function(n) {
                        a(n.form).submit(function() {
                            if (a.trim(a(n).val()) == a.data(n, j)) {
                                a(n).removeClass(h).val("")
                            }
                            return true
                        })
                    }(m)
...
jQuery(document).ready(function(b) {
    function a(c) {
        b("form.custom input:" + c).each(function() {
            var d = b('<span class="custom ' + c + '"></span>');
            if (b(this).next("span.custom." + c).length === 0) {
                if (this.checked) {
                    d.addClass("checked")
                }
                b(this).hide().after(d)
    ...
    b("form.custom select").each(function() {
        var e = b(this),g = e.next("div.custom.dropdown"),c = e.find("option"),d = 0,f;
        if (g.length === 0) {
            g = b('<div class="custom dropdown"><a href="#" class="selector"></a><ul></ul></div>"');
            c.each(function() {
                f = b("<li>" + b(this).html() + "</li>");
                g.find("ul").append(f)
            });
            g.prepend('<a href="#" class="current">' + c.first().html() + "</a>");
            e.after(g);
            e.hide()
        }
        c.each(function(h) {
            if (this.selected) {
                ...
                g.find(".current").html(b(this).html())
            }
        });
        g.find("li").each(function() {
            g.addClass("open");
            if (b(this).outerWidth() > d) {
                d = b(this).outerWidth()
            }
            g.removeClass("open")
        });
        g.css("width", d + 18 + "px");
        g.find("ul").css("width", d + 16 + "px")
    })
});
(function(b) {
    function a(d) {
    ...
    function c(d) {
        var f = d.prev(),e = f[0];
        b("input:radio[name=" + f.attr("name") + "]").each(function() {
    ...
    b("form.custom span.custom.checkbox").live("click", function(d) {
        d.preventDefault();
        d.stopPropagation();
        a(b(this))
    });
    b("form.custom span.custom.radio").live("click", function(d) {
        d.preventDefault();
        d.stopPropagation();
        c(b(this))
    });
    b("form.custom label").live("click", function(e) {
        var d = b("#" + b(this).attr("for")),g,f;
        if (d.length !== 0) {
            if (d.attr("type") === "checkbox") {
                e.preventDefault();
                g = b(this).find("span.custom.checkbox");
                a(g)
            } else {
                if (d.attr("type") === "radio") {
                    e.preventDefault();
                    f = b(this).find("span.custom.radio");
                    c(f)
                }
            }
        }
    });
    b("form.custom div.custom.dropdown a.current, form.custom div.custom.dropdown a.selector").live("click", function(d) {
        var f = b(this),e = f.closest("div.custom.dropdown");
        d.preventDefault();
        e.toggleClass("open");
        if (e.hasClass("open")) {
            b(document).bind("click.customdropdown", function(g) {
                e.removeClass("open");
    ...
    b("form.custom div.custom.dropdown li").live("click", function(g) {
        var h = b(this),e = h.closest("div.custom.dropdown"),f = e.prev(),d = 0;
        g.preventDefault();
        g.stopPropagation();
        h.closest("ul").find("li").removeClass("selected");
        h.addClass("selected");
        e.removeClass("open").find("a.current").html(h.html());
        h.closest("ul").find("li").each(function(i) {
            if (h[0] == this) {
                d = i
            }
        });
...
(function(j, i, l) {
    var k = "placeholder" in i.createElement("input"),h = "placeholder" in i.createElement("textarea");
    if (k && h) {
            ...
            if (b.is(":password")) {
                if (!b.data("placeholder-textinput")) {
                    try {
                        ...
                        a = l("<input>").attr(l.extend(n(this), {type:"text"}))
                    }
