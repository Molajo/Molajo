...
(function (f, h, c) {
    ...
    if (a && d) {
        ...
        j = i.placeholder = function () {
            return this.filter((a ? 'textarea' : ':input') + '[placeholder]').not('.placeholder').bind('focus.placeholder', b).bind('blur.placeholder', e).plugin('blur.placeholder').end()
        };
        j.input = a;
        j.textarea = d;
        c(function () {
        ...
        if (n.val() === '') {
            if (n.is(':password')) {
                if (!n.data('placeholder-textinput')) {
                    try {
                        o = n.clone().attr({type:'text'})
                    } catch (l) {
                        o = c('<input>').attr(c.extend(g(this), {type:'text'}))
                    }
