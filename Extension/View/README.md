...
* Template - views shared by resources, modules, and triggers
...
Views can be reused by any Resource, Module, or Trigger. Even Form Field Types are generated by Views.
...
* extensions/resource/view-type/view-name (also for the module and trigger folders)
...
For extensions (resources, modules, and triggers), content items and items for a specific primary categories, the catalog id can be used as a folder name to load CSS and JS files. This can be further refined by placing the catalog id folder beneath the application name folder.
...
To display one item:
    ...
    <?php echo Services::Registry()->get('Configuration', 'url_sef'); ?>
