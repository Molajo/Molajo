...
Ext.EventManager = function () {
    ...
    function n(E) {
        var H = false, D = 0, C = u.length, H = false, F = false, G;
        if (E) {
...
Ext.apply(Ext.EventManager, function () {
    ...
    }, textResizeInterval:50, ieDeferSrc:false, useKeydown:d}
}());
...
Ext.Element.addMethods(function () {
    ...
    return{adjustWidth:function (G) {
        ...
        return t && t.getComputedStyle ? function (M) {
            var J = this.dom, G, I, H, K, L = Ext.isWebKit, K;
            if (J == document) {
                return null
            }
            M = x(M);
        ...
        if (!G || !K.anim) {
            ...
            K.anim({opacity:{to:H}}, K.preanim(arguments, 1), null, 0.35, "easeIn")
        }
        return K
...
Ext.Element.addMethods({scrollTo:function (b, d, a) {
    ...
    if (!a || !c.anim) {
        ...
        h = "scroll" + (e ? "Left" : "Top"), c.anim({scroll:{to:e ? [g[h], d] : [d, g[h]]}}, c.preanim(arguments, 2), "scroll")
    }
    return c
...
Ext.Element.addMethods(function () {
    ...
    return{originalDisplay:"", visibilityMode:1, setVisibilityMode:function (m) {
        ...
        return !m[n] ? false : (typeof m[n] == "object" ? m[n] : {duration:m[n + 1], callback:m[n + 2], easing:m[n + 3]})
    }, isVisible:function () {
        ...
        if (!o || !p.anim) {
            if (n) {
                ...
                if (s) {
                    if (!r) {
                        p.hideModeStyles = {position:p.getStyle("position"), top:p.getStyle("top"), left:p.getStyle("left")};
                        p.applyStyles({position:"absolute", top:"-10000px", left:"-10000px"})
                    } else {
                        p.applyStyles(p.hideModeStyles || {position:"", top:"", left:""})
                    }
                } else {
                    p.fixDisplay();
            ...
            p.anim({opacity:{to:(r ? 1 : 0)}}, p.preanim(arguments, 1), null, 0.35, "easeIn", function () {
                if (!r) {
                    q.style[n ? d : h] = (n) ? j : b;
...
(function () {
    ...
    Ext.extend(Ext.data.Connection, Ext.util.Observable, {timeout:30000, autoAbort:false, disableCaching:true, disableCachingParam:"_dc", request:function (n) {
        ...
        if (s.fireEvent(b, s, n)) {
            ...
            var l = n.params, k = n.url || s.url, j, q = {success:s.handleResponse, failure:s.handleFailure, scope:s, argument:{options:n}, timeout:n.timeout || s.timeout}, m, t;
            if (Ext.isFunction(l)) {
...
Ext.extend(Ext.util.ClickRepeater, Ext.util.Observable, {interval:20, delay:250, preventDefault:true, stopDefault:false, timer:0, enable:function () {
    if (this.disabled) {
...
}, enter:false, left:false, right:false, up:false, down:false, tab:false, esc:false, pageUp:false, pageDown:false, del:false, home:false, end:false, keyToHandler:{37:"left", 39:"right", 38:"up", 40:"down", 33:"pageUp", 34:"pageDown", 46:"del", 36:"home", 35:"end", 13:"enter", 27:"esc", 9:"tab"}, stopKeyUp:function (b) {
    var a = b.getKey();
    if (a >= 37 && a <= 40) {
        b.stopEvent()
    }
...
Ext.reg = Ext.ResourceMgr.registerType;
Ext.preg = Ext.ResourceMgr.registerPlugin;
Ext.create = Ext.ResourceMgr.create;
Ext.Resource = function (b) {
    b = b || {};
    if (b.initialConfig) {
        if (b.isAction) {
            this.baseAction = b
    ...
    this.getId();
    Ext.ResourceMgr.register(this);
    Ext.Resource.superclass.constructor.call(this);
    if (this.baseAction) {
        this.baseAction.addResource(this)
    }
    this.initResource();
    if (this.plugins) {
...
Ext.Resource.AUTO_ID = 1000;
Ext.extend(Ext.Resource, Ext.util.Observable, {disabled:false, hidden:false, autoEl:"div", disabledClass:"x-item-disabled", allowDomMove:true, autoShow:false, hideMode:"display", hideParent:false, rendered:false, tplWriteMode:"overwrite", bubbleEvents:[], ctype:"Ext.Resource", actionMode:"el", getActionEl:function () {
    return this[this.actionMode]
}, initPlugin:function (a) {
    if (a.ptype && !Ext.isFunction(a.init)) {
        a = Ext.ResourceMgr.createPlugin(a)
    } else {
        if (Ext.isString(a)) {
            a = Ext.ResourceMgr.createPlugin({ptype:a})
        }
    }
    a.init(this);
    return a
    ...
    if (!this.isDestroyed) {
        if (this.fireEvent("beforedestroy", this) !== false) {
            ...
            this.onDestroy();
            Ext.ResourceMgr.unregister(this);
    ...
    return this.id || (this.id = "ext-comp-" + (++Ext.Resource.AUTO_ID))
}, getItemId:function () {
    ...
    Ext.Resource.superclass.purgeListeners.call(this);
    if (this.mons) {
...
Ext.Action = Ext.extend(Object, {constructor:function (a) {
    ...
    this.items.push(a);
    a.on("destroy", this.removeResource, this)
...
Ext.BoxResource = Ext.extend(Ext.Resource, {initResource:function () {
    Ext.BoxResource.superclass.initResource.call(this);
    ...
    Ext.BoxResource.superclass.afterRender.call(this);
    if (this.resizeEl) {
...
Ext.reg("box", Ext.BoxResource);
Ext.Spacer = Ext.extend(Ext.BoxResource, {autoEl:"div"});
...
Ext.Container = Ext.extend(Ext.BoxResource, {bufferResize:50, autoDestroy:true, forceLayout:false, defaultType:"panel", resizeEvent:"resize", bubbleEvents:["add", "remove"], initResource:function () {
    Ext.Container.superclass.initResource.call(this);
    ...
    if (!this.items) {
        this.items = new Ext.util.MixedCollection(false, this.getResourceId);
        this.getLayout()
    }
    ...
    var g = this.lookupResource(this.applyDefaults(b));
    ...
    var k = this.lookupResource(this.applyDefaults(g));
...
}, lookupResource:function (a) {
    if (Ext.isString(a)) {
        return Ext.ResourceMgr.get(a)
    } else {
        if (!a.events) {
            return this.createResource(a)
        }
    }
    return a
}, createResource:function (a, d) {
    if (a.render) {
        return a
    }
...
Ext.layout.AnchorLayout = Ext.extend(Ext.layout.ContainerLayout, {monitorResize:true, type:"anchor", defaultAnchor:"100%", parseAnchorRE:/^(r|right|b|bottom)$/i, getLayoutTargetSize:function () {
    ...
    for (j = 0; j < n; j++) {
        ...
        if (q.anchor) {
            ...
            if (!s) {
                u = q.anchor.split(" ");
                q.anchorSpec = s = {right:this.parseAnchor(u[0], q.initialConfig.width, d), bottom:this.parseAnchor(u[1], q.initialConfig.height, r)}
            }
            ...
            if (g || b) {
                k.push({comp:q, width:g || undefined, height:b || undefined})
            }
        }
    }
...
Ext.layout.FormLayout = Ext.extend(Ext.layout.AnchorLayout, {labelSeparator:":", trackLabels:false, type:"form", onRemove:function (d) {
...
Ext.layout.AccordionLayout = Ext.extend(Ext.layout.FitLayout, {fill:true, autoWidth:true, titleCollapse:true, hideCollapseTool:false, collapseFirst:false, animate:false, sequence:false, activeOnTop:false, type:"accordion", renderItem:function (a) {
    ...
    var a = this.activeItem;
    c = this.container.getResource(c);
    if (a != c) {
...
}, updateInnerCtSize:Ext.emptyFn, handleTargetOverflow:function (d, a, c) {
...
Ext.layout.VBoxLayout = Ext.extend(Ext.layout.BoxLayout, {align:"left", type:"vbox", updateInnerCtSize:function (b, d) {
    ...
    var k = w.length, u = this.padding, y = u.top, p = u.left, q = y + u.bottom, a = p + u.right, x = e.width - this.scrollOffset, v = e.height, A = Math.max(0, x - a), l = this.pack == "start", m = this.pack == "center", n = this.pack == "end", I = 0, E = 0, z = 0, j = [], h, d, g, b, t, C, B, s, F, G, D;
    for (B = 0; B < k; B++) {
        h = w[B];
        g = h.height;
        d = h.width;
        ...
        j.push({resource:h, height:g || undefined, width:d || undefined})
    }
    ...
    return{boxes:j, meta:{maxWidth:E}}
}});
Ext.Container.LAYOUTS.vbox = Ext.layout.VBoxLayout;
Ext.layout.HBoxLayout = Ext.extend(Ext.layout.BoxLayout, {align:"top", type:"hbox", updateInnerCtSize:function (b, d) {
    ...
    var n = z.length, x = this.padding, C = x.top, r = x.left, s = C + x.bottom, a = r + x.right, B = e.width - this.scrollOffset, y = e.height, h = Math.max(0, y - s), o = this.pack == "start", p = this.pack == "center", q = this.pack == "end", I = 0, A = 0, D = 0, m = [], l, d, g, c, w, F, E, v, H, j, t;
    for (E = 0; E < n; E++) {
        l = z[E];
        g = l.height;
        d = l.width;
        ...
        m.push({resource:l, height:g || undefined, width:d || undefined})
    }
    ...
    return{boxes:m, meta:{maxHeight:A}}
}});
...
Ext.layout.ToolbarLayout = Ext.extend(Ext.layout.ContainerLayout, {monitorResize:true, type:"toolbar", triggerWidth:18, noItemsMenuText:'<div class="x-toolbar-no-items">(None)</div>', lastOverflow:false, tableHTML:['<table cellspacing="0" class="x-toolbar-ct">', "<tbody>", "<tr>", '<td class="x-toolbar-left" align="{0}">', '<table cellspacing="0">', "<tbody>", '<tr class="x-toolbar-left-row"></tr>', "</tbody>", "</table>", "</td>", '<td class="x-toolbar-right" align="right">', '<table cellspacing="0" class="x-toolbar-right-ct">', "<tbody>", "<tr>", "<td>", '<table cellspacing="0">', "<tbody>", '<tr class="x-toolbar-right-row"></tr>', "</tbody>", "</table>", "</td>", "<td>", '<table cellspacing="0">', "<tbody>", '<tr class="x-toolbar-extras-row"></tr>', "</tbody>", "</table>", "</td>", "</tr>", "</tbody>", "</table>", "</td>", "</tr>", "</tbody>", "</table>"].join(""), onLayout:function (e, j) {
    ...
    delete b.id;
    return b
}, addResourceToMenu:function (b, a) {
...
Ext.layout.MenuLayout = Ext.extend(Ext.layout.ContainerLayout, {monitorResize:true, type:"menu", setContainer:function (a) {
    ...
    return{isMenuItem:a, needsIcon:!a && (b.icon || b.iconCls), icon:b.icon || Ext.BLANK_IMAGE_URL, iconCls:"x-menu-item-icon " + (b.iconCls || ""), itemId:"x-menu-el-" + b.id, itemCls:"x-menu-list-item "}
}, isValidParent:function (b, a) {
...
Ext.Viewport = Ext.extend(Ext.Container, {initResource:function () {
    Ext.Viewport.superclass.initResource.call(this);
...
Ext.Panel = Ext.extend(Ext.Container, {baseCls:"x-panel", collapsedCls:"x-panel-collapsed", maskDisabled:true, animCollapse:Ext.enableFx, headerAsText:true, buttonAlign:"right", collapsed:false, collapseFirst:true, minButtonWidth:75, elements:"body", preventBodyReset:false, padding:undefined, resizeEvent:"bodyresize", toolTarget:"header", collapseEl:"bwrap", slideAnchor:"t", disabledClass:"", deferHeight:true, expandDefaults:{duration:0.25}, collapseDefaults:{duration:0.25}, initResource:function () {
    ...
    a = b.events ? Ext.apply(b, c) : this.createResource(Ext.apply({}, b, c), "toolbar");
    ...
    if (this.rendered && this.header) {
        if (this.frame) {
            ...
            if (c) {
                ...
                if (d) {
                    Ext.DomHelper.insertBefore(d.dom, {tag:"img", src:Ext.BLANK_IMAGE_URL, cls:"x-panel-inline-icon " + this.iconCls})
                }
            }
        }
    }
...
Ext.extend(Ext.Editor, Ext.Resource, {allowBlur:true, value:"", alignment:"c-c?", offsets:[0, 0], shadow:"frame", constrain:false, swallowKeys:true, completeOnEnter:true, cancelOnEsc:true, updateEl:false, initResource:function () {
    Ext.Editor.superclass.initResource.call(this);
...
Ext.ColorPalette = Ext.extend(Ext.Resource, {itemCls:"x-color-palette", value:null, clickEvent:"click", ctype:"Ext.ColorPalette", allowReselect:false, colors:["000000", "993300", "333300", "003300", "003366", "000080", "333399", "333333", "800000", "FF6600", "808000", "008000", "008080", "0000FF", "666699", "808080", "FF0000", "FF9900", "99CC00", "339966", "33CCCC", "3366FF", "800080", "969696", "FF00FF", "FFCC00", "FFFF00", "00FF00", "00FFFF", "00CCFF", "993366", "C0C0C0", "FF99CC", "FFCC99", "FFFF99", "CCFFCC", "CCFFFF", "99CCFF", "CC99FF", "FFFFFF"], initResource:function () {
    Ext.ColorPalette.superclass.initResource.call(this);
...
Ext.DatePicker = Ext.extend(Ext.BoxResource, {todayText:"Today", okText:"&#160;OK&#160;", cancelText:"Cancel", todayTip:"{0} (Spacebar)", minText:"This date is before the minimum date", maxText:"This date is after the maximum date", format:"m/d/y", disabledDaysText:"Disabled", disabledDatesText:"Disabled", monthNames:Date.monthNames, dayNames:Date.dayNames, nextText:"Next Month (Control+Right)", prevText:"Previous Month (Control+Left)", monthYearText:"Choose a month (Control+Up/Down to move years)", startDay:0, showToday:true, focusOnSelect:true, initHour:12, initResource:function () {
    Ext.DatePicker.superclass.initResource.call(this);
...
Ext.ns("Ext.slider");
Ext.slider.Thumb = Ext.extend(Object, {constructor:function (a) {
...
Ext.slider.MultiSlider = Ext.extend(Ext.BoxResource, {vertical:false, minValue:0, maxValue:100, decimalPrecision:0, keyIncrement:1, increment:0, clickRange:[5, 15], clickToChange:true, animate:true, dragging:false, constrainThumbs:true, topThumbZIndex:10000, initResource:function () {
    if (!Ext.isDefined(this.value)) {
        this.value = this.minValue
    }
    this.thumbs = [];
    Ext.slider.MultiSlider.superclass.initResource.call(this);
...
Ext.ProgressBar = Ext.extend(Ext.BoxResource, {baseCls:"x-progress", animate:false, waitTimer:null, initResource:function () {
    Ext.ProgressBar.superclass.initResource.call(this);
...
Ext.extend(Ext.data.XmlReader, Ext.data.DataReader, {read:function (a) {
    ...
    var b = new Ext.data.Response({action:e, success:this.getSuccess(g), message:this.getMessage(g), data:this.extractData(d.select(this.meta.record, g) || d.select(this.meta.root, g), false), raw:g});
...
Ext.Window = Ext.extend(Ext.Panel, {baseCls:"x-window", resizable:true, draggable:true, closable:true, closeAction:"close", constrain:false, constrainHeader:false, plain:false, minimizable:false, maximizable:false, minHeight:100, minWidth:200, expandOnShow:true, collapsible:false, initHidden:undefined, hidden:true, elements:"header,body", frame:true, floating:true, initResource:function () {
    this.initTools();
    Ext.Window.superclass.initResource.call(this);
    ...
    this.proxy.shift(Ext.apply(a, {callback:this.afterShow.createDelegate(this, [true], false), scope:this, easing:"easeNone", duration:0.25, opacity:0.5}))
}, hide:function (c, a, b) {
...
Ext.DataView = Ext.extend(Ext.BoxResource, {selectedClass:"x-view-selected", emptyText:"", deferEmptyText:true, trackOver:false, blockRefresh:false, last:false, initResource:function () {
    Ext.DataView.superclass.initResource.call(this);
...
Ext.list.ListView = Ext.extend(Ext.DataView, {itemSelector:"dl", selectedClass:"x-list-selected", overClass:"x-list-over", scrollOffset:undefined, columnResize:true, columnSort:true, maxWidth:Ext.isIE ? 99 : 100, initResource:function () {
    ...
    Ext.list.ListView.superclass.initResource.call(this)
}, onRender:function () {
...
Ext.list.ColumnResizer = Ext.extend(Ext.util.Observable, {minPct:0.05, constructor:function (a) {
    ...
    this.proxy.remove();
    var j = this.hdIndex, o = this.view, h = o.columns, l = h.length, q = this.view.innerHd.getWidth(), g = this.minPct * 100, r = Math.ceil((k * o.maxWidth) / q), p = (h[j].width * 100) - r, a = Math.floor(p / (l - 1 - j)), n = p - (a * (l - 1 - j));
    for (var d = j + 1; d < l; d++) {
        var c = (h[d].width * 100) + a, b = Math.max(g, c);
        if (c != b) {
            n += c - b
        }
        h[d].width = b / 100
...
Ext.TabPanel = Ext.extend(Ext.Panel, {deferredRender:true, tabWidth:120, minTabWidth:30, resizeTabs:false, enableTabScroll:false, scrollIncrement:0, scrollRepeatInterval:400, scrollDuration:0.35, animScroll:true, tabPosition:"top", baseCls:"x-tab-panel", autoTabs:false, autoTabSelector:"div.x-tab", activeTab:undefined, tabMargin:2, plain:false, wheelIncrement:20, idDelimiter:"__", itemCls:"x-tab-item", elements:"body", headerAsText:false, frame:false, hideBorders:true, initResource:function () {
    this.frame = false;
    Ext.TabPanel.superclass.initResource.call(this);
    ...
    if (a) {
        b = this.getResource(a.id.split(this.idDelimiter)[1]);
        if (b.disabled) {
...
Ext.Button = Ext.extend(Ext.BoxResource, {hidden:false, disabled:false, pressed:false, enableToggle:false, menuAlign:"tl-bl?", type:"button", menuClassTarget:"tr:nth(2)", clickEvent:"click", handleMouseEvents:true, tooltipType:"qtip", buttonSelector:"button:first-child", scale:"small", iconAlign:"left", arrowAlign:"right", initResource:function () {
    Ext.Button.superclass.initResource.call(this);
...
Ext.SplitButton = Ext.extend(Ext.Button, {arrowSelector:"em", split:true, initResource:function () {
    Ext.SplitButton.superclass.initResource.call(this);
...
Ext.CycleButton = Ext.extend(Ext.SplitButton, {getItemText:function (a) {
    ...
    Ext.CycleButton.superclass.initResource.call(this);
...
(function () {
    var a = Ext.Toolbar;
    Ext.extend(a, Ext.Container, {defaultType:"button", enableOverflow:false, trackMenus:true, internalDefaults:{removeMode:"container", hideParent:true}, toolbarCls:"x-toolbar", initResource:function () {
        a.superclass.initResource.call(this);
            ...
            if (b.isFormField || b.render) {
                ...
                if (b.tag) {
                    ...
                    if (b.tagName) {
                        ...
                        if (Ext.isObject(b)) {
                            b = b.xtype ? this.createResource(b) : this.constructButton(b)
                        }
                    }
                }
            }
        ...
        var c = d.events ? d : this.createResource(d, d.split ? "splitbutton" : this.defaultType);
        return c
    }, onAdd:function (b) {
    ...
    a.Item = Ext.extend(Ext.BoxResource, {hideParent:true, enable:Ext.emptyFn, disable:Ext.emptyFn, focus:Ext.emptyFn});
...
Ext.ButtonGroup = Ext.extend(Ext.Panel, {baseCls:"x-btn-group", layout:"table", defaultType:"button", frame:true, internalDefaults:{removeMode:"container", hideParent:true}, initResource:function () {
    ...
    Ext.ButtonGroup.superclass.initResource.call(this)
}, applyDefaults:function (b) {
...
(function () {
    var a = Ext.Toolbar;
    Ext.PagingToolbar = Ext.extend(Ext.Toolbar, {pageSize:20, displayMsg:"Displaying {0} - {1} of {2}", emptyMsg:"No data to display", beforePageText:"Page", afterPageText:"of {0}", firstText:"First Page", prevText:"Previous Page", nextText:"Next Page", lastText:"Last Page", refreshText:"Refresh", initResource:function () {
        ...
        Ext.PagingToolbar.superclass.initResource.call(this);
...
Ext.Tip = Ext.extend(Ext.Panel, {minWidth:40, maxWidth:300, shadow:"sides", defaultAlign:"tl-bl?", autoRender:true, quickShowInterval:250, frame:true, hidden:true, baseCls:"x-tip", floating:{shadow:true, shim:true, useDisplay:true, constrain:false}, autoHeight:true, closeAction:"hide", initResource:function () {
    Ext.Tip.superclass.initResource.call(this);
...
Ext.ToolTip = Ext.extend(Ext.Tip, {showDelay:500, hideDelay:200, dismissDelay:5000, trackMouse:false, anchorToTarget:true, anchorOffset:0, targetCounter:0, constrainPosition:false, initResource:function () {
    Ext.ToolTip.superclass.initResource.call(this);
...
Ext.QuickTip = Ext.extend(Ext.ToolTip, {interceptTitles:false, tagConfig:{namespace:"ext", attribute:"qtip", width:"qwidth", target:"target", title:"qtitle", hide:"hide", cls:"qclass", align:"qalign", anchor:"anchor"}, initResource:function () {
    this.target = this.target || Ext.getDoc();
    this.targets = this.targets || {};
    Ext.QuickTip.superclass.initResource.call(this)
}, register:function (e) {
...
Ext.tree.TreePanel = Ext.extend(Ext.Panel, {rootVisible:true, animate:Ext.enableFx, lines:true, enableDD:false, hlDrop:Ext.enableFx, pathSeparator:"/", bubbleEvents:[], initResource:function () {
    Ext.tree.TreePanel.superclass.initResource.call(this);
    if (!this.eventModel) {
...
var swfobject = function () {
    ...
    function M(Z) {
        ...
        return Y && typeof encodeURIResource != E ? encodeURIResource(Z) : Z
    }
...
Ext.FlashResource = Ext.extend(Ext.BoxResource, {flashVersion:"9.0.115", backgroundColor:"#ffffff", wmode:"opaque", flashVars:undefined, flashParams:undefined, url:undefined, swfId:undefined, swfWidth:"100%", swfHeight:"100%", expressInstall:false, initResource:function () {
    Ext.FlashResource.superclass.initResource.call(this);
    this.addEvents("initialize")
}, onRender:function () {
    Ext.FlashResource.superclass.onRender.apply(this, arguments);
    ...
    new swfobject.embedSWF(this.url, this.id, this.swfWidth, this.swfHeight, this.flashVersion, this.expressInstall ? Ext.FlashResource.EXPRESS_INSTALL_URL : undefined, a, b);
    this.swf = Ext.getDom(this.id);
    this.el = Ext.get(this.swf)
}, getSwfId:function () {
    return this.swfId || (this.swfId = "extswf" + (++Ext.Resource.AUTO_ID))
}, getId:function () {
    return this.id || (this.id = "extflashcmp" + (++Ext.Resource.AUTO_ID))
    ...
    Ext.FlashResource.superclass.beforeDestroy.call(this)
}, onSwfReady:Ext.emptyFn});
Ext.FlashResource.EXPRESS_INSTALL_URL = "http://swfobject.googlecode.com/svn/trunk/swfobject/expressInstall.swf";
Ext.reg("flash", Ext.FlashResource);
...
Ext.chart.Chart = Ext.extend(Ext.FlashResource, {refreshBuffer:100, chartStyle:{padding:10, animationEnabled:true, font:{name:"Tahoma", color:4473924, size:11}, dataTip:{padding:5, border:{color:10075112, size:1}, background:{color:14346230, alpha:0.9}, font:{name:"Tahoma", color:1393291, size:10, bold:true}}}, extraStyle:null, seriesStyles:null, disableCaching:Ext.isIE || Ext.isOpera, disableCacheParam:"_dc", initResource:function () {
    Ext.chart.Chart.superclass.initResource.call(this);
    if (!this.url) {
...
Ext.menu.Menu = Ext.extend(Ext.Container, {minWidth:120, shadow:"sides", subMenuAlign:"tl-tr?", defaultAlign:"tl-bl?", allowOtherMenus:false, ignoreParentClicks:false, enableScrolling:true, maxHeight:null, scrollIncrement:24, showSeparator:true, defaultOffsets:[0, 0], plain:false, floating:true, zIndex:15000, hidden:true, layout:"menu", hideMode:"offsets", scrollerHeight:8, autoLayout:true, defaultType:"menuitem", bufferResize:false, initResource:function () {
    ...
    Ext.menu.Menu.superclass.initResource.call(this);
    if (this.autoLayout) {
...
Ext.menu.BaseItem = Ext.extend(Ext.Resource, {canActivate:false, activeClass:"x-menu-item-active", hideOnClick:true, clickHideDelay:1, ctype:"Ext.menu.BaseItem", actionMode:"container", initResource:function () {
    Ext.menu.BaseItem.superclass.initResource.call(this);
...
Ext.menu.Item = Ext.extend(Ext.menu.BaseItem, {itemCls:"x-menu-item", canActivate:true, showDelay:200, hideDelay:200, ctype:"Ext.menu.Item", initResource:function () {
    Ext.menu.Item.superclass.initResource.call(this);
    if (this.menu) {
        this.menu = Ext.menu.MenuMgr.get(this.menu);
        this.menu.ownerCt = this
    }
    ...
    return{id:this.id, cls:this.itemCls + (this.menu ? " x-menu-item-arrow" : "") + (this.cls ? " " + this.cls : ""), href:this.href || "#", hrefTarget:this.hrefTarget, icon:this.icon || Ext.BLANK_IMAGE_URL, iconCls:this.iconCls || "", text:this.itemText || this.text || "&#160;"}
}, setText:function (a) {
    this.text = a || "&#160;";
    if (this.rendered) {
        this.textEl.update(this.text);
...
Ext.menu.CheckItem = Ext.extend(Ext.menu.Item, {itemCls:"x-menu-item x-menu-check-item", groupClass:"x-menu-group-item", checked:false, ctype:"Ext.menu.CheckItem", initResource:function () {
    Ext.menu.CheckItem.superclass.initResource.call(this);
...
Ext.menu.DateMenu = Ext.extend(Ext.menu.Menu, {enableScrolling:false, hideOnClick:true, pickerId:null, cls:"x-date-menu", initResource:function () {
    ...
    Ext.menu.DateMenu.superclass.initResource.call(this);
...
Ext.menu.ColorMenu = Ext.extend(Ext.menu.Menu, {enableScrolling:false, hideOnClick:true, cls:"x-color-menu", paletteId:null, initResource:function () {
    ...
    Ext.menu.ColorMenu.superclass.initResource.call(this);
...
Ext.form.Field = Ext.extend(Ext.BoxResource, {invalidClass:"x-form-invalid", invalidText:"The value in this field is invalid", focusClass:"x-form-focus", validationEvent:"keyup", validateOnBlur:true, validationDelay:250, defaultAutoCreate:{tag:"input", type:"text", size:"20", autocomplete:"off"}, fieldClass:"x-form-field", msgTarget:"qtip", msgFx:"normal", readOnly:false, disabled:false, submitValue:true, isFormField:true, msgDisplay:"", hasFocus:false, initResource:function () {
    Ext.form.Field.superclass.initResource.call(this);
...
Ext.form.TextField = Ext.extend(Ext.form.Field, {grow:false, growMin:30, growMax:800, vtype:null, maskRe:null, disableKeyFilter:false, allowBlank:true, minLength:0, maxLength:Number.MAX_VALUE, minLengthText:"The minimum length for this field is {0}", maxLengthText:"The maximum length for this field is {0}", selectOnFocus:false, blankText:"This field is required", validator:null, regex:null, regexText:"", emptyText:null, emptyClass:"x-form-empty-field", initResource:function () {
    Ext.form.TextField.superclass.initResource.call(this);
...
Ext.form.TwinTriggerField = Ext.extend(Ext.form.TriggerField, {initResource:function () {
    Ext.form.TwinTriggerField.superclass.initResource.call(this);
...
Ext.form.DateField = Ext.extend(Ext.form.TriggerField, {format:"m/d/Y", altFormats:"m/d/Y|n/j/Y|n/j/y|m/j/y|n/d/y|m/j/Y|n/d/Y|m-d-y|m-d-Y|m/d|m-d|md|mdy|mdY|d|Y-m-d", disabledDaysText:"Disabled", disabledDatesText:"Disabled", minText:"The date in this field must be equal to or after {0}", maxText:"The date in this field must be equal to or before {0}", invalidText:"{0} is not a valid date - it must be in the format {1}", triggerClass:"x-form-date-trigger", showToday:true, defaultAutoCreate:{tag:"input", type:"text", size:"10", autocomplete:"off"}, initTime:"12", initTimeFormat:"H", safeParse:function (b, c) {
    ...
    Ext.form.DateField.superclass.initResource.call(this);
    this.addEvents("select");
...
Ext.form.ComboBox = Ext.extend(Ext.form.TriggerField, {defaultAutoCreate:{tag:"input", type:"text", size:"24", autocomplete:"off"}, listClass:"", selectedClass:"x-combo-selected", listEmptyText:"", triggerClass:"x-form-arrow-trigger", shadow:"sides", listAlign:"tl-bl?", maxHeight:300, minHeight:90, triggerAction:"query", minChars:4, autoSelect:true, typeAhead:false, queryDelay:500, pageSize:0, selectOnFocus:false, queryParam:"query", loadingText:"Loading...", resizable:false, handleHeight:8, allQuery:"", mode:"remote", minListWidth:70, forceSelection:false, typeAheadDelay:250, lazyInit:true, clearFilterOnReset:true, submitValue:undefined, initResource:function () {
    Ext.form.ComboBox.superclass.initResource.call(this);
    ...
    if (this.transform) {
        ...
        if (!this.store) {
            ...
            this.store = new Ext.data.ArrayStore({id:0, fields:["value", "text"], data:j, autoDestroy:true});
    ...
    if (!this.list) {
        var a = "x-combo-list", d = Ext.getDom(this.getListParent() || Ext.getBody()), b = parseInt(Ext.fly(d).getStyle("z-index"), 10);
        if (!b) {
            b = this.getParentZIndex()
        }
        this.list = new Ext.Layer({parentEl:d, shadow:this.shadow, cls:[a, this.listClass].join(" "), constrain:false, zindex:(b || 12000) + 5});
    ...
    this.list.alignTo.apply(this.list, [this.el].concat(this.listAlign));
    var b = Ext.getDom(this.getListParent() || Ext.getBody()), a = parseInt(Ext.fly(b).getStyle("z-index"), 10);
    if (!a) {
...
Ext.form.Checkbox = Ext.extend(Ext.form.Field, {focusClass:undefined, fieldClass:"x-form-field", checked:false, boxLabel:"&#160;", defaultAutoCreate:{tag:"input", type:"checkbox", autocomplete:"off"}, actionMode:"wrap", initResource:function () {
    Ext.form.Checkbox.superclass.initResource.call(this);
...
Ext.form.CheckboxGroup = Ext.extend(Ext.form.Field, {columns:"auto", vertical:false, allowBlank:true, blankText:"You must select at least one item in this group", defaultType:"checkbox", groupCls:"x-form-check-group", initResource:function () {
    this.addEvents("change");
    this.on("change", this.validate, this);
    Ext.form.CheckboxGroup.superclass.initResource.call(this)
...
Ext.form.CompositeField = Ext.extend(Ext.form.Field, {defaultMargins:"0 5 0 0", skipLastItemMargin:true, isComposite:true, combineErrors:true, initResource:function () {
    var e = [], a = this.items, d;
    for (var c = 0, b = a.length; c < b; c++) {
        d = a[c];
        e.push(d.fieldLabel);
        Ext.apply(d, this.defaults);
        if (!(c == b - 1 && this.skipLastItemMargin)) {
            Ext.applyIf(d, {margins:this.defaultMargins})
        }
    }
    ...
    Ext.form.CompositeField.superclass.initResource.apply(this, arguments)
}, onRender:function (d, b) {
    if (!this.el) {
        var e = this.innerCt = new Ext.Container({layout:"hbox", renderTo:d, items:this.items, cls:"x-form-composite", defaultMargins:"0 3 0 0"});
        this.el = e.getEl();
        ...
        if (this.combineErrors) {
            this.eachItem(function (g) {
                Ext.apply(g, {markInvalid:this.onFieldMarkInvalid.createDelegate(this, [g], 0), clearInvalid:this.onFieldClearInvalid.createDelegate(this, [g], 0)})
            })
        }
    ...
    var b = d.getName(), a = {field:b, error:c};
...
Ext.FormPanel = Ext.extend(Ext.Panel, {minButtonWidth:75, labelAlign:"left", monitorValid:false, monitorPoll:200, layout:"form", initResource:function () {
    this.form = this.createForm();
    Ext.FormPanel.superclass.initResource.call(this);
...
Ext.form.HtmlEditor = Ext.extend(Ext.form.Field, {enableFormat:true, enableFontSize:true, enableColors:true, enableAlignments:true, enableLists:true, enableSourceEdit:true, enableLinks:true, enableFont:true, createLinkText:"Please enter the URL for the link:", defaultLinkValue:"http://", fontFamilies:["Arial", "Courier New", "Tahoma", "Times New Roman", "Verdana"], defaultFont:"tahoma", defaultValue:(Ext.isOpera || Ext.isIE6) ? "&#160;" : "&#8203;", actionMode:"wrap", validationEvent:false, deferHeight:true, initialized:false, activated:false, sourceEditMode:false, onFocus:Ext.emptyFn, iframePad:3, hideMode:"offsets", defaultAutoCreate:{tag:"textarea", style:"width:500px;height:300px;", autocomplete:"off"}, initResource:function () {
    ...
    var d = this.tb.getResource("sourceedit");
...
Ext.form.TimeField = Ext.extend(Ext.form.ComboBox, {minValue:undefined, maxValue:undefined, minText:"The time in this field must be equal to or after {0}", maxText:"The time in this field must be equal to or before {0}", invalidText:"{0} is not a valid time", format:"g:i A", altFormats:"g:ia|g:iA|g:i a|g:i A|h:i|g:i|H:i|ga|ha|gA|h a|g a|g A|gi|hi|gia|hia|g|H|gi a|hi a|giA|hiA|gi A|hi A", increment:15, mode:"local", triggerAction:"all", typeAhead:false, initDate:"1/1/2008", initDateFormat:"j/n/Y", initResource:function () {
    ...
    Ext.form.TimeField.superclass.initResource.call(this)
...
Ext.form.SliderField = Ext.extend(Ext.form.Field, {useTips:true, tipText:null, actionMode:"wrap", initResource:function () {
    ...
    Ext.form.SliderField.superclass.initResource.call(this)
}, onRender:function (b, a) {
...
Ext.form.Label = Ext.extend(Ext.BoxResource, {onRender:function (b, a) {
    if (!this.el) {
...
Ext.extend(Ext.form.Action.Submit, Ext.form.Action, {type:"submit", run:function () {
    ...
    if (d.clientValidation === false || this.form.isValid()) {
        ...
        Ext.Ajax.request(Ext.apply(this.createCallback(d), {form:this.form.el.dom, url:this.getUrl(c), method:e, headers:d.headers, params:!c ? this.getParams() : null, isUpload:this.form.fileUpload}));
        if (d.submitEmptyText === false) {
...
Ext.grid.GridView = Ext.extend(Ext.util.Observable, {deferEmptyText:true, scrollOffset:undefined, autoFill:false, forceFit:false, sortClasses:["sort-asc", "sort-desc"], sortAscText:"Sort Ascending", sortDescText:"Sort Descending", columnsText:"Columns", selectedRowClass:"x-grid3-row-selected", borderWidth:2, tdClass:"x-grid3-cell", hdCls:"x-grid3-hd", markDirty:true, cellSelectorDepth:4, rowSelectorDepth:10, rowBodySelectorDepth:10, cellSelector:"td.x-grid3-cell", rowSelector:"div.x-grid3-row", rowBodySelector:"div.x-grid3-row-body", firstRowCls:"x-grid3-row-first", lastRowCls:"x-grid3-row-last", rowClsRe:/(?:^|\s+)x-grid3-row-(first|last|alt)(?:\s+|$)/g, constructor:function (a) {
    ...
    var e = this.templates, c = e.cell, y = e.row, o = r - 1;
    var b = "width:" + this.getTotalWidth() + ";";
    var k = [], l = [], n = {tstyle:b}, q = {}, h, g;
    ...
    var c = this.templates, e = this.renderHeaders(), a = c.body.apply({rows:"&#160;"});
    var b = c.master.apply({body:a, header:e, ostyle:"width:" + this.getOffsetWidth() + ";", bstyle:"width:" + this.getTotalWidth() + ";"});
    var d = this.grid;
    ...
    if (d.enableDragDrop || d.enableDrag) {
        this.dragZone = new Ext.grid.GridDragZone(d, {ddGroup:d.ddGroup || "GridDD"})
    }
    this.updateHeaderSortState()
}, processEvent:function (c, i) {
    var d = i.getTarget(), h = this.grid, k = this.findHeaderIndex(d);
    h.fireEvent(c, i);
    if (k !== false) {
        h.fireEvent("header" + c, h, k, i)
    } else {
        var j = this.findRowIndex(d), b, a;
        if (j !== false) {
    ...
    var c = this.cm, h = this.templates, e = h.hcell, b = [], j = {}, a = c.getColumnCount(), g = a - 1;
    for (var d = 0; d < a; d++) {
    ...
    return h.header.apply({cells:b.join(""), tstyle:"width:" + this.getTotalWidth() + ";"})
    ...
    for (var c = 0; c < e; c++) {
        ...
        d[c] = {name:(!Ext.isDefined(b) ? this.ds.fields.get(c).name : b), renderer:a.getRenderer(c), scope:a.getRendererScope(c), id:a.getColumnId(c), style:this.getColumnStyle(c)}
    }
    return d
    ...
    if (b) {
        b.on({scope:this, load:this.onLoad, datachanged:this.onDataChange, add:this.onAdd, remove:this.onRemove, update:this.onUpdate, clear:this.onClear})
    }
    this.ds = b;
    if (this.cm) {
    ...
    for (var b = 0; b < c; b++) {
        if (a.config[b].hideable !== false) {
            this.colMenu.add(new Ext.menu.CheckItem({itemId:"col-" + a.getColumnId(b), text:a.getColumnHeader(b), checked:!a.isHidden(b), hideOnClick:false, disabled:a.config[b].hideable === false}))
        }
    }
    ...
    if (h && !this.headersDisabled) {
        var b = this.splitHandleWidth || 5, g = this.activeHdRegion, a = i.getPageX(), c = h.style, j = "";
...
Ext.grid.GridView.SplitDragZone = Ext.extend(Ext.dd.DDProxy, {constructor:function (a, b) {
    ...
    if (i && this.allowHeaderDrag(a)) {
        var l = this.view.fly(i).getXY(), d = l[0], c = l[1], j = a.getXY(), b = j[0], h = i.offsetWidth, g = false;
        if ((b - d) <= this.hw) {
            g = -1
        } else {
            if ((d + h) - b <= this.hw) {
...
Ext.grid.RowSelectionModel = Ext.extend(Ext.grid.AbstractSelectionModel, {singleSelect:false, constructor:function (a) {
    ...
    this.rowNav = new Ext.KeyNav(this.grid.getGridEl(), {up:function (b) {
...
Ext.grid.Column = Ext.extend(Object, {isColumn:true, constructor:function (b) {
    Ext.apply(this, b);
...
Ext.grid.CheckboxSelectionModel = Ext.extend(Ext.grid.RowSelectionModel, {header:'<div class="x-grid3-hd-checker">&#160;</div>', width:20, sortable:false, menuDisabled:true, fixed:true, hideable:false, dataIndex:"", id:"checker", constructor:function () {
...
Ext.grid.EditorGridPanel = Ext.extend(Ext.grid.GridPanel, {clicksToEdit:2, forceValidation:false, isEditor:true, detectEdit:false, autoEncode:false, trackMouseOver:false, initResource:function () {
    Ext.grid.EditorGridPanel.superclass.initResource.call(this);
    if (!this.selModel) {
...
Ext.grid.PropertyGrid = Ext.extend(Ext.grid.EditorGridPanel, {enableColumnMove:false, stripeRows:false, trackMouseOver:false, clicksToEdit:1, enableHdMenu:false, viewConfig:{forceFit:true}, initResource:function () {
    ...
    this.cm = a;
    this.ds = b.store;
    Ext.grid.PropertyGrid.superclass.initResource.call(this);
...
Ext.grid.GroupingView = Ext.extend(Ext.grid.GridView, {groupByText:"Group By This Field", showGroupsText:"Show in Groups", hideGroupedColumn:false, showGroupName:true, startCollapsed:false, enableGrouping:true, enableGroupingMenu:true, enableNoGroups:true, emptyGroupText:"(None)", ignoreAdd:false, groupTextTpl:"{text}", groupMode:"value", initTemplates:function () {
    ...
    if (g) {
        var d = this.getGroupField(), c = this.getPrefix(d), a = g.id.substring(c.length);
