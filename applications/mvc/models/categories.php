...
class MolajoModelCategories
{
    ...
    /**
     * Returns a reference to a MolajoCategoriesHelpers object
     *
    ...
    /**
     * Loads a specific category and all its children in a MolajoCategoryNode object
     *
        ...
        if (count($results)) {
            ...
            foreach ($results as $result)
            {
                ...
                if (!isset($this->_nodes[$result->id])) {
                    // Create the MolajoCategoryNode and add to _nodes
                    $this->_nodes[$result->id] = new MolajoCategoryNode($result, $this);
...
class MolajoCategoryNode extends JObject
{
    ...
    {
        if ($parent instanceof MolajoCategoryNode || is_null($parent)) {
    ...
    {
        if ($child instanceof MolajoCategoryNode) {
     ...
     *
     * @param   object   $sibling  MolajoCategoryNode object for the sibling
     ...
     *
     * @return  MolajoCategoryNode or null  MolajoCategoryNode object with the sibling information or
     ...
     *
     * @return  MolajoUser    A MolajoUser object containing a user_id
     */
    function getAuthor($modified_user = false)
    {
        if ($modified_user) {
            return Molajo::User($this->modified_user_id);
        }

        return Molajo::User($this->created_user_id);
    }
