...
abstract class MolajoHtmlList
{
    ...
    public static function stateOptions($config = array())
    {
        ...
        if (!array_key_exists('archived', $config) || $config['archived']) {
            $options[] = MolajoHTML::_('select.option', '2', MolajoTextHelper::_('MOLAJO_OPTION_STATUS_ARCHIVED'));
        }
        if (!array_key_exists('published', $config) || $config['published']) {
            $options[] = MolajoHTML::_('select.option', '1', MolajoTextHelper::_('MOLAJO_OPTION_STATUS_PUBLISHED'));
        }
        if (!array_key_exists('unpublished', $config) || $config['unpublished']) {
            $options[] = MolajoHTML::_('select.option', '0', MolajoTextHelper::_('MOLAJO_OPTION_STATUS_UNPUBLISHED'));
        }
        if ($parameters->def(state_spam, 0) == '1') {
            if (!array_key_exists('spam', $config) || $config['spam']) {
                $options[] = MolajoHTML::_('select.option', '-1', MolajoTextHelper::_('MOLAJO_OPTION_STATUS_SPAMMED'));
            }
        }
        if (!array_key_exists('trash', $config) || $config['trash']) {
            $options[] = MolajoHTML::_('select.option', '-2', MolajoTextHelper::_('MOLAJO_OPTION_STATUS_TRASHED'));
        }
        if ($parameters->def(version_management, 1) == '1') {
            if (!array_key_exists('version', $config) || $config['version']) {
                $options[] = MolajoHTML::_('select.option', '-10', MolajoTextHelper::_('MOLAJO_OPTION_STATUS_VERSION'));
            }
        }
