...
abstract class MolajoHtmlMenu
{
    ...
    public static function menuitems($config = array())
    {
        if (empty(self::$items)) {
            ...
            $query->clear();
            $query->select('a.id AS value, a.title AS text, a.lvl, a.menutype');
            ...
            foreach ($menus as &$menu)
            {
                ...
                self::$items[] = MolajoHTML::_('select.option', $menu->value . '.1', MolajoTextHelper::_('JLIB_HTML_ADD_TO_THIS_MENU'));
    ...
    public static function ordering(&$row, $id)
    {
        ...
        {
            $ordering = '<input type="hidden" name="ordering" value="' . $row->ordering . '" />' . MolajoTextHelper::_('JGLOBAL_NEWITEMSLAST_DESC');
        }
    ...
    public static function linkoptions($all = false, $unassigned = false)
    {
        ...
        $mitems = array();
        if ($all | $unassigned) {
            $mitems[] = MolajoHTML::_('select.option', '<OPTGROUP>', MolajoTextHelper::_('JOPTION_MENUS'));

            if ($all) {
                $mitems[] = MolajoHTML::_('select.option', 0, MolajoTextHelper::_('JALL'));
            }
            if ($unassigned) {
                $mitems[] = MolajoHTML::_('select.option', -1, MolajoTextHelper::_('JOPTION_UNASSIGNED'));
            }
