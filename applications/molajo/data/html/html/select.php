 ...
 * @since       11.1
 */
abstract class MolajoHtmlSelect
{
    /**
    ...
    {
        $arr = array(MolajoHTML::_('select.option', '0', MolajoTextHelper::_($no)), MolajoHTML::_('select.option', '1', MolajoTextHelper::_($yes)));
     ...
     *                               Format options, see {@see MolajoHTML::$formatOptions}.
     *                               Selection options, see {@see MolajoHTMLSelect::options()}.
    ...
    {
        // Set default options
        $options = array_merge(MolajoHTML::$formatOptions, array('format.depth' => 0, 'id' => false));
     ...
     *                            Format options, {@see MolajoHTML::$formatOptions}.
    ...
    {
        // Set default options
        $options = array_merge(MolajoHTML::$formatOptions, array('format.depth' => 0, 'option.format' => '', 'id' => null));
        ...
        return MolajoHTML::_('select.genericlist', $data, $name, $options);
    }
     ...
     *                                of selected options. Default: none.
     *                               -list.translate: Boolean. If set, text and labels are translated via
     *                                MolajoTextHelper::_(). Default is false.
    ...
    public static function options($arr, $optKey = 'value', $optText = 'text', $selected = null, $translate = false)
    {
        ...
        foreach ($arr as $elementKey => &$element)
        {
            ...
            if ($options['groups'] && $key == '<OPTGROUP>') {
                $html .= $baseIndent . '<optgroup label="' . ($options['list.translate'] ? MolajoTextHelper::_($text)
            ...
            else
            {
                ...
                if ($options['list.translate'] && !empty($label)) {
                    $label = MolajoTextHelper::_($label);
                }
        ...
        $translate = false)
    {
        ...
        {
            $k = $obj->$optKey;
            $t = $translate ? MolajoTextHelper::_($obj->$optText) : $obj->$optText;
