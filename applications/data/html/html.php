...
abstract class MolajoHtml
{
    ...
    protected static function extract($key)
    {
        ...
        $prefix = (count($parts) == 3 ? array_shift($parts) : 'MolajoHTML');
    ...
    public static function _($key)
    {
        ...
        if (is_callable($toCall)) {
            MolajoHTML::register($key, $toCall);
            $args = func_get_args();
            // Remove function name from arguments
            array_shift($args);
            return MolajoHTML::call($toCall, $args);
        }
        else
        {
    ...
    public static function stylesheet($file, $attribs = array(), $relative = false, $path_only = false, $detect_browser = true)
    {
        ...
        else
        {
            ...
            {
                MolajoController::getApplication()->addStylesheetLinks($include, 'text/css', null, $attribs);
            }
        }
    }
    ...
    public static function script($file, $framework = false, $relative = false, $path_only = false, $detect_browser = true)
    {
        ...
        else
        {
            foreach ($includes as $include)
            {
                MolajoController::getApplication()->addJavascriptLink($include);
            }
        }
    }
    ...
    public static function core($debug = null)
    {
        ...
        if ($debug === null) {
            $debug = MolajoController::getApplication()->get('debug');
        }

        $uncompressed = $debug ? '-uncompressed' : '';

        MolajoController::getApplication()->addJavascriptLink(JURI::root(true) . '/media/system/js/core' . $uncompressed . '.js');
    }
     ...
     * array. See {@see MolajoHTML::$formatOptions} for details.
     *
    ...
    public static function date($input = 'now', $format = null, $tz = true, $gregorian = false)
    {
        ...
        if ($tz === true) {
            ...
            $date = MolajoController::getDate($input, 'UTC');
        ...
        elseif ($tz === null)
        {
            $date = MolajoController::getDate($input);
        }
    ...
    public static function calendar($value, $name, $id, $format = '%Y-%m-%d', $attribs = null)
    {
        ...
        if ((!$readonly) && (!$disabled)) {
            ...
            MolajoHTML::_('behavior.calendar');
    ...
    /**
     * Add a directory where MolajoHTML should search for helpers. You may
    ...
    public static function addIncludePath($path = '')
    {
        ...
        foreach ($path as $dir)
        {
            if (!empty($dir) && !in_array($dir, MolajoHTML::$includePaths)) {
                jimport('joomla.filesystem.path');
                array_unshift(MolajoHTML::$includePaths, JPath::clean($dir));
            }
        }

        return MolajoHTML::$includePaths;
    }
}
