...
abstract class MolajoHtmlBehavior
{
    ...
    public static function caption($selector = 'img.caption')
    {
        ...
        $uncompressed = MolajoFactory::getApplication()->getConfig->get('debug') ? '-uncompressed' : '';
    ...
    public static function tooltip($selector = '.hasTip', $parameters = array())
    {
        ...
        $opt['offset'] = (isset($parameters['offset']) && (is_array($parameters['offset']))) ? $parameters['offset']
                : null;
        if (!isset($opt['offset'])) {
            // Suppporting offsets parameter which was working in mootools 1.2 (Joomla!1.5)
            $opt['offset'] = (isset($parameters['offsets']) && (is_array($parameters['offsets'])))
                    ? $parameters['offsets'] : null;
        }
    ...
    public static function uploader($id = 'file-upload', $parameters = array(), $upload_queue = 'upload-queue')
    {
        ...
        if (!isset($uploaders)) {
            ...
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_FILENAME');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_UPLOAD_COMPLETED');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_ERROR_OCCURRED');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_ALL_FILES');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_PROGRESS_OVERALL');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_CURRENT_TITLE');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_REMOVE');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_REMOVE_TITLE');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_CURRENT_FILE');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_CURRENT_PROGRESS');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_FILE_ERROR');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_FILE_SUCCESSFULLY_UPLOADED');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_DUPLICATE');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_SIZELIMITMIN');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_SIZELIMITMAX');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_FILELISTMAX');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_FILELISTSIZEMAX');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_ERROR_HTTPSTATUS');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_ERROR_SECURITYERROR');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_ERROR_IOERROR');
            MolajoTextHelper::script('JLIB_HTML_BEHAVIOR_UPLOADER_ALL_FILES');
        }
        ...
        $onFileSuccess = '\\function(file, response) {
			...
			if (json.get(\'status\') == \'1\') {
				...
				file.info.set(\'html\', \'<strong>\' + Joomla.MolajoTextHelper._(\'JLIB_HTML_BEHAVIOR_UPLOADER_FILE_SUCCESSFULLY_UPLOADED\') + \'</strong>\');
			} else {
				file.element.addClass(\'file-failed\');
				file.info.set(\'html\', \'<strong>\' +
					Joomla.MolajoTextHelper._(\'JLIB_HTML_BEHAVIOR_UPLOADER_ERROR_OCCURRED\',
        ...
        $opt['path'] = (isset($parameters['swf'])) ? $parameters['swf']
                : JURI::root(true) . '/media/system/swf/uploader.swf';
        ...
        $opt['allowDuplicates'] = (isset($parameters['allowDuplicates']) && !($parameters['allowDuplicates'])) ? false
                : true;
        // limitSize is the old parameter name.  Remove in 1.7
        $opt['fileSizeMax'] = (isset($parameters['limitSize']) && ($parameters['limitSize']))
                ? (int)$parameters['limitSize']
                : null;
        // fileSizeMax is the new name.  If supplied, it will override the old value specified for limitSize
        $opt['fileSizeMax'] = (isset($parameters['fileSizeMax']) && ($parameters['fileSizeMax']))
                ? (int)$parameters['fileSizeMax']
                : $opt['fileSizeMax'];
        $opt['fileSizeMin'] = (isset($parameters['fileSizeMin']) && ($parameters['fileSizeMin']))
                ? (int)$parameters['fileSizeMin']
                : null;
        // limitFiles is the old parameter name.  Remove in 1.7
        $opt['fileListMax'] = (isset($parameters['limitFiles']) && ($parameters['limitFiles']))
                ? (int)$parameters['limitFiles']
                : null;
        // fileListMax is the new name.  If supplied, it will override the old value specified for limitFiles
        $opt['fileListMax'] = (isset($parameters['fileListMax']) && ($parameters['fileListMax']))
                ? (int)$parameters['fileListMax']
                : $opt['fileListMax'];
        ...
        $opt['typeFilter'] = (isset($parameters['types'])) ? '\\' . $parameters['types']
                : '\\{Joomla.MolajoTextHelper._(\'JLIB_HTML_BEHAVIOR_UPLOADER_ALL_FILES\'): \'*.*\'}';
        $opt['typeFilter'] = (isset($parameters['typeFilter'])) ? '\\' . $parameters['typeFilter'] : $opt['typeFilter'];

        // Optional functions
        $opt['createReplacement'] = (isset($parameters['createReplacement'])) ? '\\' . $parameters['createReplacement']
                : null;
        ...
        $opt['onFileSuccess'] = (isset($parameters['onFileSuccess'])) ? '\\' . $parameters['onFileSuccess']
                : $onFileSuccess;
    ...
    protected static function _calendartranslation()
    {
        ...
        if ($jsscript == 0) {
            $return = 'Calendar._DN = new Array ("' . MolajoTextHelper::_('SUNDAY', true) . '", "' . MolajoTextHelper::_('MONDAY', true) . '", "'
                      . MolajoTextHelper::_('TUESDAY', true) . '", "' . MolajoTextHelper::_('WEDNESDAY', true) . '", "' . MolajoTextHelper::_('THURSDAY', true) . '", "'
                      . MolajoTextHelper::_('FRIDAY', true) . '", "' . MolajoTextHelper::_('SATURDAY', true) . '", "' . MolajoTextHelper::_('SUNDAY', true) . '");'
                      . ' Calendar._SDN = new Array ("' . MolajoTextHelper::_('SUN', true) . '", "' . MolajoTextHelper::_('MON', true) . '", "' . MolajoTextHelper::_('TUE', true) . '", "'
                      . MolajoTextHelper::_('WED', true) . '", "' . MolajoTextHelper::_('THU', true) . '", "' . MolajoTextHelper::_('FRI', true) . '", "' . MolajoTextHelper::_('SAT', true) . '", "'
                      . MolajoTextHelper::_('SUN', true) . '");' . ' Calendar._FD = 0;' . ' Calendar._MN = new Array ("' . MolajoTextHelper::_('JANUARY', true) . '", "'
                      . MolajoTextHelper::_('FEBRUARY', true) . '", "' . MolajoTextHelper::_('MARCH', true) . '", "' . MolajoTextHelper::_('APRIL', true) . '", "' . MolajoTextHelper::_('MAY', true)
                      . '", "' . MolajoTextHelper::_('JUNE', true) . '", "' . MolajoTextHelper::_('JULY', true) . '", "' . MolajoTextHelper::_('AUGUST', true) . '", "'
                      . MolajoTextHelper::_('SEPTEMBER', true) . '", "' . MolajoTextHelper::_('OCTOBER', true) . '", "' . MolajoTextHelper::_('NOVEMBER', true) . '", "'
                      . MolajoTextHelper::_('DECEMBER', true) . '");' . ' Calendar._SMN = new Array ("' . MolajoTextHelper::_('JANUARY_SHORT', true) . '", "'
                      . MolajoTextHelper::_('FEBRUARY_SHORT', true) . '", "' . MolajoTextHelper::_('MARCH_SHORT', true) . '", "' . MolajoTextHelper::_('APRIL_SHORT', true) . '", "'
                      . MolajoTextHelper::_('MAY_SHORT', true) . '", "' . MolajoTextHelper::_('JUNE_SHORT', true) . '", "' . MolajoTextHelper::_('JULY_SHORT', true) . '", "'
                      . MolajoTextHelper::_('AUGUST_SHORT', true) . '", "' . MolajoTextHelper::_('SEPTEMBER_SHORT', true) . '", "' . MolajoTextHelper::_('OCTOBER_SHORT', true) . '", "'
                      . MolajoTextHelper::_('NOVEMBER_SHORT', true) . '", "' . MolajoTextHelper::_('DECEMBER_SHORT', true) . '");'
                      . ' Calendar._TT = {};Calendar._TT["INFO"] = "' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_ABOUT_THE_CALENDAR', true) . '";'
                      . ' Calendar._TT["ABOUT"] =
...
"\n\n" +
"' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_DATE_SELECTION', true) . '" +
"' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_YEAR_SELECT', true) . '" +
"' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_MONTH_SELECT', true) . '" +
"' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_HOLD_MOUSE', true)
                      . '";
...
"- or click and drag for faster selection.";
		...
		Calendar._TT["PREV_YEAR"] = "' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_PREV_YEAR_HOLD_FOR_MENU', true) . '";' . ' Calendar._TT["PREV_MONTH"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_PREV_MONTH_HOLD_FOR_MENU', true) . '";' . ' Calendar._TT["GO_TODAY"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_GO_TODAY', true) . '";' . ' Calendar._TT["NEXT_MONTH"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_NEXT_MONTH_HOLD_FOR_MENU', true) . '";' . ' Calendar._TT["NEXT_YEAR"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_NEXT_YEAR_HOLD_FOR_MENU', true) . '";' . ' Calendar._TT["SEL_DATE"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_SELECT_DATE', true) . '";' . ' Calendar._TT["DRAG_TO_MOVE"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_DRAG_TO_MOVE', true) . '";' . ' Calendar._TT["PART_TODAY"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_TODAY', true) . '";' . ' Calendar._TT["DAY_FIRST"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_DISPLAY_S_FIRST', true) . '";' . ' Calendar._TT["WEEKEND"] = "0,6";' . ' Calendar._TT["CLOSE"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_CLOSE', true) . '";' . ' Calendar._TT["TODAY"] = "' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_TODAY', true)
                      . '";' . ' Calendar._TT["TIME_PART"] = "' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_SHIFT_CLICK_OR_DRAG_TO_CHANGE_VALUE', true) . '";'
                      . ' Calendar._TT["DEF_DATE_FORMAT"] = "%Y-%m-%d";' . ' Calendar._TT["TT_DATE_FORMAT"] = "'
                      . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_TT_DATE_FORMAT', true) . '";' . ' Calendar._TT["WK"] = "' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_WK', true) . '";'
                      . ' Calendar._TT["TIME"] = "' . MolajoTextHelper::_('JLIB_HTML_BEHAVIOR_TIME', true) . '";';
            $jsscript = 1;
            return $return;
        }
        else
