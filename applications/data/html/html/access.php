...
abstract class MolajoHtmlAccess
{
    ...
    public static function level($name, $selected, $attribs = '', $parameters = true, $id = false)
    {
        ...
        elseif ($parameters) {
            array_unshift($options, MolajoHTML::_('select.option', '', MolajoTextHelper::_('JOPTION_ACCESS_SHOW_ALL_LEVELS')));
        }

        return MolajoHTML::_('select.genericlist', $options, $name,
                             array(
                                  'list.attr' => $attribs,
    ...
    public static function usergroup($name, $selected, $attribs = '', $allowAll = true)
    {
        ...
        if ($allowAll) {
            array_unshift($options, MolajoHTML::_('select.option', '', MolajoTextHelper::_('JOPTION_ACCESS_SHOW_ALL_GROUPS')));
        }
    ...
    public static function usergroups($name, $selected, $checkSuperAdmin = false)
    {
        ...
        $isSuperAdmin = MolajoController::getUser()->authorise('admin');
    ...
    public static function actions($name, $selected, $component, $section = 'global')
    {
        ...
        for ($i = 0, $n = count($actions); $i < $n; $i++) {
            ...
            $html[] = '			' . $checked . ' />';
            $html[] = '		<label for="' . $eid . '">';
            $html[] = '			' . MolajoTextHelper::_($item->title);
            $html[] = '		</label>';
            $html[] = '	</li>';
        }
        $html[] = '</ul>';
    ...
    /**
     * Gets a list of the asset groups as an array of MolajoHTML compatible options.
     *
    ...
    {
        if (empty(MolajoHTMLAccess::$asset_groups)) {
            $db = MolajoController::getDbo();
            $query = $db->getQuery(true);
            ...
            $db->setQuery($query);
            MolajoHTMLAccess::$asset_groups = $db->loadObjectList();
        ...
        return MolajoHTMLAccess::$asset_groups;
    }
    ...
    public static function assetgrouplist($name, $selected, $attribs = null, $config = array())
    {
        ...
        $options = MolajoHTMLAccess::assetgroups();
        if (isset($config['title'])) {
            array_unshift($options, MolajoHTML::_('select.option', '', $config['title']));
        }
