 ...
 * @since       1.0
 */
class MolajoResponseService extends Response
{
    /**
     * Response instance
     *
    ...
    public static function getInstance($content = '', $status = 200, $headers = array())
    {
        if (empty(self::$instance)) {
            self::$instance = new MolajoResponseService($content, $status, $headers);
        }
    ...
    public function isRedirect($location = null)
    {
        ...
        if (Services::Configuration()->get('debug', 0) == 1) {
            debug('MolajoResponseer::isRedirect redirect to: ' . $location);
        }
