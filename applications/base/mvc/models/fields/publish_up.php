 ...
 * @since      1.6
 */
class MolajoFieldPublishUp extends MolajoField
{
    /**
     *  __construct
     *
    ...
    public function getQueryInformation($query, $value, $selectedState, $onlyWhereClause = false)
    {
        ...
        $db = $this->getDb();
        $query->where('SUBSTRING(a.start_publishing_datetime, 1, 7) = ' . $db->q(substr($value, 0, 4) . '-' . substr($value, 4, 2)));
    }
    ...
    public function render($view, $item, $itemCount)
    {
        if ($view == 'admin') {
            ...
            if ($item->start_publishing_datetime == 0) {
                ...
                $render['print_value'] = JHTML::_('date', $item->start_publishing_datetime, Services::Language()->translate('DATE_FORMAT_LC4'));
            }
