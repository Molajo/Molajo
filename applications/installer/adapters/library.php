...
class MolajoInstallerAdapterLibrary extends MolajoAdapterInstance
{
    ...
    public function install()
    {
        ...
        if ($result) {
            ...
            else
            {
                ...
                $this->parent->abort(TextServices::_('JLIB_INSTALLER_ABORT_LIB_INSTALL_ALREADY_INSTALLED'));
                return false;
            }
        }
        ...
        if (!$group) {
            $this->parent->abort(TextServices::_('JLIB_INSTALLER_ABORT_LIB_INSTALL_NOFILE'));
            return false;
        }
        else
        {
        ...
        if (!file_exists($this->parent->getPath('extension_root'))) {
            if (!$created = JFolder::create($this->parent->getPath('extension_root'))) {
                $this->parent->abort(
                    TextServices::sprintf('JLIB_INSTALLER_ABORT_LIB_INSTALL_FAILED_TO_CREATE_DIRECTORY', $this->parent->getPath('extension_root'))
                );
                return false;
            }
        }
        ...
        if (!$row->store()) {
            // Install failed, roll back changes
            $this->parent->abort(TextServices::sprintf('JLIB_INSTALLER_ABORT_LIB_INSTALL_ROLLBACK', $db->stderr(true)));
            return false;
        }
        ...
        if (!$this->parent->copyFiles(array($manifest), true)) {
            ...
            $this->parent->abort(TextServices::_('JLIB_INSTALLER_ABORT_LIB_INSTALL_COPY_SETUP'));
            return false;
        }
    ...
    public function uninstall($id)
    {
        ...
        if (!$row->load((int)$id) || !strlen($row->element)) {
            MolajoError::raiseWarning(100, TextServices::_('ERRORUNKOWNEXTENSION'));
            return false;
        }
        ...
        if ($row->protected) {
            MolajoError::raiseWarning(100, TextServices::_('JLIB_INSTALLER_ERROR_LIB_UNINSTALL_WARNCORELIBRARY'));
            return false;
        }
        ...
        if (file_exists($manifestFile)) {
            ...
            if (!$xml) {
                MolajoError::raiseWarning(100, TextServices::_('JLIB_INSTALLER_ERROR_LIB_UNINSTALL_LOAD_MANIFEST'));
                return false;
            }
        ...
        else
        {
            ...
            unset($row);
            MolajoError::raiseWarning(100, TextServices::_('JLIB_INSTALLER_ERROR_LIB_UNINSTALL_INVALID_NOTFOUND_MANIFEST'));
            return false;
        }
    ...
    public function discover_install()
    {
        ...
        {
            MolajoError::raiseWarning(101, TextServices::_('JLIB_INSTALLER_ERROR_LIB_DISCOVER_STORE_DETAILS'));
            return false;
        }
    }
    ...
    public function refreshManifestCache()
    {
        ...
        {
            MolajoError::raiseWarning(101, TextServices::_('JLIB_INSTALLER_ERROR_LIB_REFRESH_MANIFEST_CACHE'));
            return false;
        }
    }
}
