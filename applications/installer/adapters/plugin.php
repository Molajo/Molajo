 ...
 * @since       11.1
 */
class MolajoAdapterPlugin extends MolajoAdapterInstance
{
    /**
    ...
    public function install()
    {
        ...
        {
            $this->parent->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_PLG_INSTALL_NO_FILE', Services::Language()->translate('JLIB_INSTALLER_' . $this->route)));
            return false;
        }
        ...
        catch (Exception $e)
        {
            ...
            $this->parent
                    ->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_PLG_INSTALL_ROLLBACK', Services::Language()->translate('JLIB_INSTALLER_' . $this->route), $db->stderr(true)));
            return false;
        }
        $id = $db->loadResult();
            ...
            elseif (!$this->parent->getOverwrite())
            {
                ...
                $this->parent
                    ...
                    Services::Language()->sprintf(
                        'JLIB_INSTALLER_ABORT_PLG_INSTALL_DIRECTORY', Services::Language()->translate('JLIB_INSTALLER_' . $this->route),
        ...
        if ($this->parent->manifestClass && method_exists($this->parent->manifestClass, 'preflight')) {
            if ($this->parent->manifestClass->preflight($this->route, $this) === false) {
                ...
                $this->parent->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_PLG_INSTALL_CUSTOM_INSTALL_FAILURE'));
                return false;
            }
        }
        ...
        if (!file_exists($this->parent->getPath('extension_root'))) {
            if (!$created = Services::Folder()->create($this->parent->getPath('extension_root'))) {
                $this->parent
                        ->abort(
                    Services::Language()->sprintf(
                        'JLIB_INSTALLER_ABORT_PLG_INSTALL_CREATE_DIRECTORY', Services::Language()->translate('JLIB_INSTALLER_' . $this->route),
                        $this->parent->getPath('extension_root')
                    )
                );
                return false;
        ...
        if ($this->get('manifest_script')) {
            ...
            if (!file_exists($path['dest'])) {
                if (!$this->parent->copyFiles(array($path))) {
                    // Install failed, rollback changes
                    $this->parent
                            ->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_PLG_INSTALL_MANIFEST', Services::Language()->translate('JLIB_INSTALLER_' . $this->route)));
                    return false;
                }
            }
        }
        ...
        if ($id) {
            if (!$this->parent->getOverwrite()) {
                ...
                $this->parent
                        ->abort(
                    Services::Language()->sprintf(
                        'JLIB_INSTALLER_ABORT_PLG_INSTALL_ALLREADY_EXISTS', Services::Language()->translate('JLIB_INSTALLER_' . $this->route),
                        $this->get('name')
                    )
                );
                return false;
        ...
        else
        {
            ...
            if (!$row->store()) {
                ...
                $this->parent
                        ->abort(
                    Services::Language()->sprintf('JLIB_INSTALLER_ABORT_PLG_INSTALL_ROLLBACK', Services::Language()->translate('JLIB_INSTALLER_' . $this->route), $db->stderr(true))
                );
                return false;
            }
        ...
        if (strtolower($this->route) == 'install') {
            ...
            if ($utfresult === false) {
                ...
                $this->parent
                        ->abort(
                    Services::Language()->sprintf('JLIB_INSTALLER_ABORT_PLG_INSTALL_SQL_ERROR', Services::Language()->translate('JLIB_INSTALLER_' . $this->route), $db->stderr(true))
                );
                return false;
            }
        ...
        elseif (strtolower($this->route) == 'update')
        {
            if ($this->manifest->update) {
                ...
                if ($result === false) {
                    // Install failed, rollback changes
                    $this->parent->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_PLG_UPDATE_SQL_ERROR', $db->stderr(true)));
                    return false;
                }
            }
        }
        ...
        if (!$this->parent->copyManifest(-1)) {
            // Install failed, rollback changes
            $this->parent->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_PLG_INSTALL_COPY_SETUP', Services::Language()->translate('JLIB_INSTALLER_' . $this->route)));
            return false;
        }
    ...
    public function uninstall($id)
    {
        ...
        $row = MolajoModel::getInstance('extension');
        if (!$row->load((int)$id)) {
            MolajoError::raiseWarning(100, Services::Language()->translate('JLIB_INSTALLER_ERROR_PLG_UNINSTALL_ERRORUNKOWNEXTENSION'));
            return false;
        }
        ...
        if ($row->protected) {
            MolajoError::raiseWarning(100, Services::Language()->sprintf('JLIB_INSTALLER_ERROR_PLG_UNINSTALL_WARNCOREPLUGIN', $row->name));
            return false;
        }

        // Get the plugin folder so we can properly build the plugin path
        if (trim($row->folder) == '') {
            MolajoError::raiseWarning(100, Services::Language()->translate('JLIB_INSTALLER_ERROR_PLG_UNINSTALL_FOLDER_FIELD_EMPTY'));
            return false;
        }
        ...
        if (!file_exists($manifestFile)) {
            MolajoError::raiseWarning(100, Services::Language()->translate('JLIB_INSTALLER_ERROR_PLG_UNINSTALL_INVALID_NOTFOUND_MANIFEST'));
            return false;
        }
        ...
        if (!$xml) {
            MolajoError::raiseWarning(100, Services::Language()->translate('JLIB_INSTALLER_ERROR_PLG_UNINSTALL_LOAD_MANIFEST'));
            return false;
        }
        ...
        if ($xml->getName() != 'extension') {
            MolajoError::raiseWarning(100, Services::Language()->translate('JLIB_INSTALLER_ERROR_PLG_UNINSTALL_INVALID_MANIFEST'));
            return false;
        }
        ...
        if ($utfresult === false) {
            // Install failed, rollback changes
            $this->parent->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_PLG_UNINSTALL_SQL_ERROR', $db->stderr(true)));
            return false;
        }
    ...
    public function discover_install()
    {
        ...
        $manifest_details = InstallHelper::parseManifestXML($manifestPath);
        ...
        {
            MolajoError::raiseWarning(101, Services::Language()->translate('JLIB_INSTALLER_ERROR_PLG_DISCOVER_STORE_DETAILS'));
            return false;
        }
    }
    ...
    public function refreshManifestCache()
    {
        ...
        {
            MolajoError::raiseWarning(101, Services::Language()->translate('JLIB_INSTALLER_ERROR_PLG_REFRESH_MANIFEST_CACHE'));
            return false;
        }
    }
}
