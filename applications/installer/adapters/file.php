...
class MolajoAdapterFile extends MolajoAdapterInstance
{
    ...
    public function install()
    {
        ...
        else
        {
            ...
            if (!$row->store()) {
                // Install failed, roll back changes
                $this->parent->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_FILE_INSTALL_ROLLBACK', $db->stderr(true)));
                return false;
            }
        ...
        elseif (strtolower($this->route) == 'update')
        {
            if ($this->manifest->update) {
                ...
                if ($result === false) {
                    // Install failed, rollback changes
                    $this->parent->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_FILE_UPDATE_SQL_ERROR', $db->stderr(true)));
                    return false;
                }
            }
        }
    ...
    public function uninstall($id)
    {
        ...
        if (file_exists($manifestFile)) {
            ...
            if ($utfresult === false) {
                // Install failed, rollback changes
                MolajoError::raiseWarning(100, Services::Language()->sprintf('JLIB_INSTALLER_ERROR_FILE_UNINSTALL_SQL_ERROR', $db->stderr(true)));
                $retval = false;
            }
    ...
    protected function extensionExistsInSystem($extension = null)
    {
        ...
        {
            // Install failed, roll back changes
            $this->parent->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_FILE_ROLLBACK', $db->stderr(true)));
            return false;
        }
        $id = $db->loadResult();
