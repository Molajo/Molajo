...
class MolajoAdapterComponent extends MolajoAdapterInstance
{
    ...
    public function install()
    {
        ...
        parent::set('message', MolajoTextService::_((string)$this->manifest->description));
        ...
        if (!$this->manifest->administration) {
            MolajoError::raiseWarning(1, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_INSTALL_ADMINISTER_ELEMENT'));
            return false;
        }
        ...
        if (file_exists(parent::getPath('extension_site')) || file_exists(parent::getPath('extension_administrator'))) {
            ...
            elseif (!parent::getOverwrite())
            {
                ...
                if (file_exists(parent::getPath('extension_site'))) {
                    ...
                    MolajoError::raiseWarning(1, MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_COMP_INSTALL_DIR_SITE', parent::getPath('extension_site')));
                }
                else
                {
                    ...
                    MolajoError::raiseWarning(
                        1,
                        MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_COMP_INSTALL_DIR_ADMIN', parent::getPath('extension_administrator'))
                    );
                }
                return false;
            }
        ...
        if (parent::manifestClass && method_exists(parent::manifestClass, 'preflight')) {
            if (parent::manifestClass->preflight('install', $this) === false) {
                ...
                parent::abort(MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_INSTALL_CUSTOM_INSTALL_FAILURE'));
                return false;
            }
        }
        ...
        if (!file_exists(parent::getPath('extension_site'))) {
            if (!$created = JFolder::create(parent::getPath('extension_site'))) {
                MolajoError::raiseWarning(
                    1,
                    MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_COMP_INSTALL_FAILED_TO_CREATE_DIRECTORY_SITE', parent::getPath('extension_site'))
                );
                return false;
            }
        }
        ...
        if ($installFile) {
            ...
            if (!file_exists(parent::getPath('extension_administrator') . '/' . $installFile) || parent::getOverwrite()) {
                ...
                if (!parent::copyFiles(array($path))) {
                    // Install failed, rollback changes
                    parent::abort(MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_INSTALL_PHP_INSTALL'));
        ...
        if ($uninstallFile) {
            ...
            if (!file_exists(parent::getPath('extension_administrator') . '/' . $uninstallFile) || parent::getOverwrite()) {
                ...
                if (!parent::copyFiles(array($path))) {
                    // Install failed, rollback changes
                    parent::abort(MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_INSTALL_PHP_UNINSTALL'));
                    return false;
                }
            }
        }
        ...
        if ($this->get('manifest_script')) {
            ...
            if (!file_exists($path['dest']) || parent::getOverwrite()) {
                if (!parent::copyFiles(array($path))) {
                    // Install failed, rollback changes
                    parent::abort(MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_INSTALL_MANIFEST'));
        ...
        if (isset($this->manifest->install->sql)) {
            ...
            if ($utfresult === false) {
                // Install failed, rollback changes
                parent::abort(MolajoTextService::sprintf('JLIB_INSTALLER_ABORT_COMP_INSTALL_SQL_ERROR', $db->stderr(true)));
        ...
        if (!$row->store()) {
            ...
            parent::abort(MolajoTextService::sprintf('JLIB_INSTALLER_ABORT_COMP_INSTALL_ROLLBACK', $db->stderr(true)));
            return false;
        }
        ...
        if (!parent::copyManifest()) {
            // Install failed, rollback changes
            parent::abort(MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_INSTALL_COPY_SETUP'));
            return false;
        }

        // Time to build the admin menus
        if (!$this->_buildAdminMenus($row->extension_id)) {
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_BUILDADMINMENUS_FAILED'));
    ...
    public function update()
    {
        ...
        if ($description) {
            parent::set('message', MolajoTextService::_($description));
        }
        else
        {
            parent::set('message', '');
        ...
        // Make sure that we have an admin element
        if (!$this->manifest->administration) {
            MolajoError::raiseWarning(1, MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_UPDATE_ADMINISTER_ELEMENT'));
            return false;
        }
        ...
        if (!file_exists(parent::getPath('extension_site'))) {
            if (!$created = JFolder::create(parent::getPath('extension_site'))) {
                MolajoError::raiseWarning(
                    1,
                    MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_COMP_UPDATE_FAILED_TO_CREATE_DIRECTORY_SITE', parent::getPath('extension_site'))
                );
        ...
        if ($installFile) {
            ...
            if (!file_exists(parent::getPath('extension_administrator') . '/' . $installFile) || parent::getOverwrite()) {
                ...
                if (!parent::copyFiles(array($path))) {
                    // Install failed, rollback changes
                    parent::abort(MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_UPDATE_PHP_INSTALL'));
                    return false;
                }
            }
        ...
        if ($uninstallFile) {
            ...
            if (!file_exists(parent::getPath('extension_administrator') . '/' . $uninstallFile) || parent::getOverwrite()) {
                ...
                if (!parent::copyFiles(array($path))) {
                    // Install failed, rollback changes
                    parent::abort(MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_UPDATE_PHP_UNINSTALL'));
        ...
        if ($this->get('manifest_script')) {
            ...
            if (!file_exists($path['dest']) || parent::getOverwrite()) {
                if (!parent::copyFiles(array($path))) {
                    // Install failed, rollback changes
                    parent::abort(MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_UPDATE_MANIFEST'));
        ...
        if ($this->manifest->update) {
            ...
            if ($result === false) {
                // Install failed, rollback changes
                parent::abort(MolajoTextService::sprintf('JLIB_INSTALLER_ABORT_COMP_UPDATE_SQL_ERROR', $db->stderr(true)));
        ...
        if (!$row->store()) {
            // Install failed, roll back changes
            parent::abort(MolajoTextService::sprintf('JLIB_INSTALLER_ABORT_COMP_UPDATE_ROLLBACK', $db->stderr(true)));
        ...
        if (!parent::copyManifest()) {
            // Install failed, rollback changes
            parent::abort(MolajoTextService::_('JLIB_INSTALLER_ABORT_COMP_UPDATE_COPY_SETUP'));
    ...
    public function uninstall($id)
    {
        ...
        $row = MolajoModel::getInstance('extension');
        if (!$row->load((int)$id)) {
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_UNINSTALL_ERRORUNKOWNEXTENSION'));
            return false;
        }
        ...
        if ($row->protected) {
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_UNINSTALL_WARNCORECOMPONENT'));
            return false;
        }
        ...
        if (!$this->manifest) {
            ...
            // Raise a warning
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_UNINSTALL_ERRORREMOVEMANUALLY'));
        ...
        if ($uninstallFile) {
            ...
            if (is_file(parent::getPath('extension_administrator') . '/' . $uninstallFile)) {
                ...
                if (function_exists('uninstall')) {
                    if (uninstall() === false) {
                        MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_UNINSTALL_CUSTOM'));
                        $retval = false;
                    }
                }
        ...
        if (isset($this->manifest->uninstall->sql)) {
            ...
            if ($utfresult === false) {
                // Install failed, rollback changes
                MolajoError::raiseWarning(100, MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_COMP_UNINSTALL_SQL_ERROR', $db->stderr(true)));
                $retval = false;
            }
        }
        ...
        // Check for errors.
        if ($db->getErrorNum()) {
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_UNINSTALL_FAILED_DELETE_CATEGORIES'));
        ...
        if (trim($row->element)) {
            ...
            if (is_dir(parent::getPath('extension_site'))) {
                if (!JFolder::delete(parent::getPath('extension_site'))) {
                    MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_UNINSTALL_FAILED_REMOVE_DIRECTORY_SITE'));
                    $retval = false;
                }
            }
            ...
            if (is_dir(parent::getPath('extension_administrator'))) {
                if (!JFolder::delete(parent::getPath('extension_administrator'))) {
                    MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_UNINSTALL_FAILED_REMOVE_DIRECTORY_ADMIN'));
                    $retval = false;
                }
            }
    ...
    protected function _removeAdminMenus(&$row)
    {
        ...
        if ($error = $db->getErrorMsg()) {
            MolajoError::raiseWarning('', MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_REMOVING_ADMINISTER_MENUS_FAILED'));
    ...
    public function discover_install()
    {
        ...
        {
            MolajoError::raiseWarning(101, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_DISCOVER_STORE_DETAILS'));
            return false;
        }
        ...
        if ($description) {
            parent::set('message', MolajoTextService::_((string)$description));
        }
        else
        {
            parent::set('message', '');
    ...
    public function refreshManifestCache()
    {
        ...
        {
            MolajoError::raiseWarning(101, MolajoTextService::_('JLIB_INSTALLER_ERROR_COMP_REFRESH_MANIFEST_CACHE'));
            return false;
        }
    }
}
