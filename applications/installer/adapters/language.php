 ...
 * @since       11.1
 */
class MolajoAdapterLanguage extends MolajoAdapterInstance
{
    /**
    ...
    public function install()
    {
        ...
        if ((string)$this->manifest->attributes()->client == 'both') {
            MolajoError::raiseWarning(42, MolajoTextService::_('JLIB_INSTALLER_ERROR_DEPRECATED_FORMAT'));
        ...
        elseif ($cname = (string)$this->manifest->attributes()->client)
        {
            ...
            if ($client === null) {
                $this->parent->abort(MolajoTextService::sprintf('JLIB_INSTALLER_ABORT', MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_UNKNOWN_CLIENT_TYPE', $cname)));
                return false;
            }
    ...
    protected function _install($cname, $basePath, $clientId, &$element)
    {
        ...
        if (!$tag) {
            $this->parent->abort(MolajoTextService::sprintf('JLIB_INSTALLER_ABORT', MolajoTextService::_('JLIB_INSTALLER_ERROR_NO_LANGUAGE_TAG')));
            return false;
        }
        ...
        if (!$this->_core) {
            if (!JFile::exists($this->parent->getPath('extension_site') . '/' . $this->get('tag') . '.xml')) {
                $this->parent
                        ->abort(MolajoTextService::sprintf('JLIB_INSTALLER_ABORT', MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_NO_CORE_LANGUAGE', $this->get('tag'))));
                return false;
            }
        }
            ...
            if (!$created = JFolder::create($this->parent->getPath('extension_site'))) {
                $this->parent
                    ...
                    MolajoTextService::sprintf(
                        ...
                        MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_CREATE_FOLDER_FAILED', $this->parent->getPath('extension_site'))
                    )
                );
                return false;
            }
            ...
            elseif (!$this->parent->getOverwrite())
            {
                ...
                if (file_exists($this->parent->getPath('extension_site'))) {
                    ...
                    MolajoError::raiseWarning(
                        ...
                        MolajoTextService::sprintf(
                            'JLIB_INSTALLER_ABORT',
                            MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_FOLDER_IN_USE', $this->parent->getPath('extension_site'))
                        )
                    );
                }
                else
                {
                    ...
                    MolajoError::raiseWarning(
                        ...
                        MolajoTextService::sprintf(
                            'JLIB_INSTALLER_ABORT',
                            MolajoTextService::sprintf('JLIB_INSTALLER_ERROR_FOLDER_IN_USE', $this->parent->getPath('extension_administrator'))
                        )
                    );
                }
                return false;
        ...
        if (!$row->store()) {
            ...
            $this->parent->abort(MolajoTextService::sprintf('JLIB_INSTALLER_ABORT', $row->getError()));
            return false;
        }
    ...
    public function uninstall($eid)
    {
        ...
        if (empty($element)) {
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_LANG_UNINSTALL_ELEMENT_EMPTY'));
            return false;
        }
        ...
        if ($protected == 1) {
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_LANG_UNINSTALL_PROTECTED'));
            return false;
        }
        ...
        if ($parameters->get($client->name) == $element) {
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_LANG_UNINSTALL_DEFAULT'));
            return false;
        }
        ...
        if (!JFolder::exists($path)) {
            ...
            $extension->delete();
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_LANG_UNINSTALL_PATH_EMPTY'));
            return false;
        }

        if (!JFolder::delete($path)) {
            // If deleting failed we'll leave the extension entry in tact just in case
            MolajoError::raiseWarning(100, MolajoTextService::_('JLIB_INSTALLER_ERROR_LANG_UNINSTALL_DIRECTORY'));
            return false;
        }
        ...
        if (!empty($count)) {
            MolajoError::raiseNotice(500, MolajoTextService::plural('JLIB_INSTALLER_NOTICE_LANG_RESET_USERS', $count));
        }
    ...
    public function discover()
    {
        ...
        foreach ($site_languages as $language)
        {
            if (file_exists(MOLAJO_BASE_FOLDER . '/language/' . $language . '/' . $language . '.xml')) {
                $manifest_details = InstallHelper::parseManifestXML(MOLAJO_BASE_FOLDER . '/language/' . $language . '/' . $language . '.xml');
                $extension = MolajoModel::getInstance('extension');
    ...
    public function discover_install()
    {
        ...
        {
            MolajoError::raiseWarning(101, MolajoTextService::_('JLIB_INSTALLER_ERROR_LANG_DISCOVER_STORE_DETAILS'));
            return false;
        }
