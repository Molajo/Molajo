...
class MolajoInstaller extends MolajoAdapter
{
    ...
    public function install($path = null)
    {
        ...
        if ($this->setupInstall()) {
            ...
            $this->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_DETECTMANIFEST'));
            return false;
        }
    ...
    public function discover_install($eid = null)
    {
        if ($eid) {
            ...
            if (!$this->extension->load($eid)) {
                $this->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_LOAD_DETAILS'));

                return false;
            }

            if ($this->extension->state != -1) {
                $this->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_ALREADYINSTALLED'));
...
//                    );
                ...
                {
                    $this->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_METHODNOTSUPPORTED'));
        ...
        {
            $this->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_EXTENSIONNOTVALID'));
    ...
    public function update($path = null)
    {
        ...
        {
            $this->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_NOUPDATEPATH'));
        }

        if (!$this->setupInstall()) {
            return $this->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_DETECTMANIFEST'));
        }
...
//            );
    ...
    function refreshManifestCache($eid)
    {
        if ($eid) {
            ...
            if ($this->extension->state == -1) {
                $this->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_REFRESH_MANIFEST_CACHE'));
                return false;
            }
            ...
            if (is_object($this->_adapters[$this->extension->type])) {
                ...
                {
                    $this->abort(Services::Language()->sprintf('JLIB_INSTALLER_ABORT_METHODNOTSUPPORTED_TYPE', $this->extension->type));
        ...
        {
            $this->abort(Services::Language()->translate('JLIB_INSTALLER_ABORT_REFRESH_MANIFEST_CACHE_VALID'));
    ...
    public function parseQueries($element)
    {
        ...
        foreach ($queries as $query)
        {
            ...
            if (!$db->query()) {
                MolajoError::raiseWarning(1, Services::Language()->sprintf('JLIB_INSTALLER_ERROR_SQL_ERROR', $db->stderr(true)));
        ...
        foreach ($element->children() as $file)
        {
            ...
            if ($fCharset == $dbCharset && $fDriver == $dbDriver) {
                ...
                if (!file_exists($sqlfile)) {
                    MolajoError::raiseWarning(1, Services::Language()->sprintf('JLIB_INSTALLER_ERROR_SQL_FILENOTFOUND', $sqlfile));
                ...
                if ($buffer === false) {
                    MolajoError::raiseWarning(1, Services::Language()->translate('JLIB_INSTALLER_ERROR_SQL_READBUFFER'));
            ...
            foreach ($files as $file)
            {
                ...
                if (!file_exists($filesource)) {
                    /*
                          ...
                          * and return false.
                          */
                    MolajoError::raiseWarning(1, Services::Language()->sprintf('JLIB_INSTALLER_ERROR_NO_FILE', $filesource));
                ...
                elseif (($exists = file_exists($filedest)) && !$overwrite)
                {
                    ...
                    MolajoError::raiseWarning(1, Services::Language()->sprintf('JLIB_INSTALLER_ERROR_FILE_EXISTS', $filedest));
