...
class MolajoComponentRenderer extends MolajoRenderer
{
    ...
    protected function _setRequest()
    {
        ...
        /** extension */
        $this->task_request->set('extension_instance_id',
        ...
        $this->task_request->set('template_view_asset_id',
        ...
        $this->task_request->set('wrap_view_asset_id',
        ...
        $this->task_request->set('mvc_id',
            (int) Molajo::Request()->get('mvc_id'));
        $this->task_request->set('mvc_category_id',
            (int) Molajo::Request()->get('mvc_category_id'));
        $this->task_request->set('mvc_suppress_no_results',
    ...
    {
        $this->task_request->set('extension_asset_type_id', MOLAJO_ASSET_TYPE_EXTENSION_COMPONENT);
        ...
        $this->task_request->set('extension_path',
            ComponentHelper::getPath(
                strtolower($this->task_request->get('extension_instance_name'))));

        $this->task_request->set('extension_type', 'component');
    ...
    protected function _importClasses()
    {
        ...
        $name = ucfirst($this->task_request->get('extension_instance_name'));
        $name = str_replace (array('-', '_'), '', $name);

        /** Controllers */
        if (file_exists($this->task_request->get('extension_path') . '/controller.php')) {
            $fileHelper->requireClassFile($this->task_request->get('extension_path') . '/controller.php', $name . 'Controller');
        }
        $files = JFolder::files($this->task_request->get('extension_path') . '/controllers', '\.php$', false, false);
        if ($files) {
            foreach ($files as $file) {
                $fileHelper->requireClassFile($this->task_request->get('extension_path') . '/controllers/' . $file, $name . 'Controller' . ucfirst(substr($file, 0, strpos($file, '.'))));
            }
        }

        /** Helpers */
        $files = JFolder::files($this->task_request->get('extension_path') . '/helpers', '\.php$', false, false);
        if ($files) {
            foreach ($files as $file) {
                $fileHelper->requireClassFile($this->task_request->get('extension_path') . '/helpers/' . $file, $name . ucfirst(substr($file, 0, strpos($file, '.'))));
            }
        }

        /** Models */
        $files = JFolder::files($this->task_request->get('extension_path') . '/models', '\.php$', false, false);
        if ($files) {
            foreach ($files as $file) {
                $fileHelper->requireClassFile($this->task_request->get('extension_path') . '/models/' . $file, $name . 'Model' . ucfirst(substr($file, 0, strpos($file, '.'))));
            }
        }

        /** Tables */
        $files = JFolder::files($this->task_request->get('extension_path') . '/tables', '\.php$', false, false);
        if ($files) {
            foreach ($files as $file) {
                $fileHelper->requireClassFile($this->task_request->get('extension_path') . '/tables/' . $file, $name . 'Table' . ucfirst(substr($file, 0, strpos($file, '.'))));
            }
        }

        /** Views */
        $folders = JFolder::folders($this->task_request->get('extension_path') . '/views', false, false);
        if ($folders) {
            foreach ($folders as $folder) {
                $files = JFolder::files($this->task_request->get('extension_path') . '/views/' . $folder, false, false);
                if ($files) {
                    foreach ($files as $file) {
                        $fileHelper->requireClassFile($this->task_request->get('extension_path') . '/views/' . $folder . '/' . $file, $name . 'View' . ucfirst($folder));
                    }
                }
            }
        }
    ...
    {
        /**  Primary Category */
        $this->_loadMediaPlus('/category' . $this->task_request->get('mvc_category_id'),
            Molajo::Application()->get('media_priority_primary_category', 700));

        /** Menu Item */
        $this->_loadMediaPlus('/menuitem' . $this->task_request->get('menu_item_id'),
            Molajo::Application()->get('media_priority_menu_item', 800));

        /** Source */
        $this->_loadMediaPlus('/source' . $this->task_request->get('mvc_id'),
            Molajo::Application()->get('media_priority_source_data', 900));

        /** Component */
        $this->_loadMediaPlus('/component' . $this->task_request->get('extension_instance_name'),
            Molajo::Application()->get('media_priority_source_data', 900));
    ...
    {
        /** Theme */
        $filePath = MOLAJO_EXTENSIONS_THEMES . '/' . $this->task_request->get('theme_name');
        $urlPath = MOLAJO_EXTENSIONS_THEMES_URL . '/' . $this->task_request->get('theme_name');
