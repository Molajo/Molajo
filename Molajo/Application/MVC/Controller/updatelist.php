...
class UpdatelistController extends UpdateController
{
    ...
    public function processItems($column = null)
    {
        ...
        if (empty($idArray)) {
            $this->redirectClass->setRedirectMessage(Service::Language()->translate('MOLAJO_BATCH_SELECT_ITEMS_TASK'));
        ...
        if ($task == 'copy' || $task == 'delete') {
            ...
            if ((int)$this->batch_category_id == 0) {
                $this->redirectClass->setRedirectMessage(Service::Language()->translate('MOLAJO_BATCH_SELECT_CATEGORY_FOR_MOVE_OR_COPY'));
                $this->redirectClass->setRedirectMessageType('message');
                return $this->redirectClass->setSuccessIndicator(false);
            }
        }
...
//        $results = $this->cleanCache();
        ...
        if ($errorFoundForBatch === false) {
            $this->redirectClass->setRedirectMessage(Service::Language()->plural('MOLAJO_N_ITEMS_' . strtoupper($task), count($idArray)));
            ...
            $this->redirectClass->setRedirectMessage(Service::Language()->translate('MOLAJO_ERROR_PROCESSING_ITEMS'));
