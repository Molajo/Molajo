 ...
 * @since      1.6
 */
ClassFieldState extends MolajoField
{
    /**
     *  __construct
     *
    ...
    public function getOptions()
    {
        ...
        $options = array();
        $options[] = MolajoHTML::_('select.option', MOLAJO_STATUS_ARCHIVED, Service::Language()->translate('MOLAJO_OPTION_STATUS_ARCHIVED'));
        $options[] = MolajoHTML::_('select.option', MOLAJO_STATUS_PUBLISHED, Service::Language()->translate('MOLAJO_OPTION_STATUS_PUBLISHED'));
        $options[] = MolajoHTML::_('select.option', MOLAJO_STATUS_UNPUBLISHED, Service::Language()->translate('MOLAJO_OPTION_STATUS_UNPUBLISHED'));
        $options[] = MolajoHTML::_('select.option', MOLAJO_STATUS_TRASHED, Service::Language()->translate('MOLAJO_OPTION_STATUS_TRASHED'));
        if ($this->parameters->get('state_spam', '0') == 1) {
            $options[] = MolajoHTML::_('select.option', MOLAJO_STATUS_SPAMMED, Service::Language()->translate('MOLAJO_OPTION_STATUS_SPAMMED'));
        }
        if ($this->parameters->get('version_management', '1') == 1) {
            $options[] = MolajoHTML::_('select.option', MOLAJO_STATUS_VERSION, Service::Language()->translate('MOLAJO_OPTION_STATUS_VERSION'));
        }
        $options[] = MolajoHTML::_('select.option', '*', Service::Language()->translate('MOLAJO_OPTION_ALL'));
