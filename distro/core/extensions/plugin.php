...
abstract class MolajoPlugin extends JEvent
{
    ...
    protected static function _import($plugin, $autocreate = true, $dispatcher = null)
    {
        ...
        $path = MOLAJO_DISTRO_PLUGINS . '/' . $plugin->folder . '/' . $plugin->title . '/' . $plugin->title . '.php';
     ...
     * @since   11.1
     */
    public function loadLanguage($extension = '', $basePath = MOLAJO_DISTRO_PLUGINS)
    {
        if (empty($extension)) {
            $extension = 'plg' . ucfirst($this->_type) . ucfirst($this->_name);
        }
        MolajoFactory::getLanguage()->load(strtolower($extension), MOLAJO_DISTRO_PLUGINS . '/' . $this->_type . '/' . $extension, null, false, false);
    }
}