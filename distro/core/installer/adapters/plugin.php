...
class MolajoInstallerAdapterPlugin extends MolajoAdapterInstance
{
    ...
    public function loadLanguage($path = null)
    {
        ...
        if (!$source) {
            $this->parent->setPath('source', MOLAJO_DISTRO_PLUGINS . '/' . $this->parent->extension->folder . '/' . $this->parent->extension->element);
        }
        ...
        if ($element) {
            ...
            if ($name) {
                ...
                $source = $path ? $path : MOLAJO_DISTRO_PLUGINS . "/$group/$name";
    ...
    public function install()
    {
        ...
        if (!empty($element) && !empty($group)) {
            $this->parent->setPath('extension_root', MOLAJO_DISTRO_PLUGINS . '/' . $group . '/' . $element);
        }
        else
        {
    ...
    public function uninstall($id)
    {
        ...
        if (is_dir(MOLAJO_DISTRO_PLUGINS . '/' . $row->folder . '/' . $row->element)) {
            // Use 1.6 plugins
            $this->parent->setPath('extension_root', MOLAJO_DISTRO_PLUGINS . '/' . $row->folder . '/' . $row->element);
        }
        else
        {
            // Use Legacy 1.5 plugins
            $this->parent->setPath('extension_root', MOLAJO_DISTRO_PLUGINS . '/' . $row->folder);
        }
        ...
        $this->parent->setPath('source', MOLAJO_DISTRO_PLUGINS . '/' . $row->folder . '/' . $row->element);
        $this->loadLanguage(MOLAJO_DISTRO_PLUGINS . '/' . $row->folder . '/' . $row->element);
