...
class modContentHelper
{
    ...
    public static function getList($parameters, $user)
    {
        ...
        if ($db->getErrorNum()) {
            JError::raiseWarning(500, MolajoTextHelper::sprintf('MOLAJO_APPLICATION_ERROR_MODULE_LOAD', $db->getErrorMsg()));
            return false;
        }
        ...
        if (count($items) == 0) {
            ...
            $items[0]->columnheading1 = MolajoTextHelper::_('LATEST_LATEST_ITEMS');
            $items[0]->columnheading2 = MolajoTextHelper::_('JSTATUS');
            $items[0]->columnheading3 = MolajoTextHelper::_('LATEST_CREATED');
            $items[0]->columnheading4 = MolajoTextHelper::_('LATEST_CREATED_BY');
            ...
            foreach ($items as $item) {
                ...
                /** Headings */
                $item->columncount = '4';
                $item->columnheading . $i = MolajoTextHelper::_('LATEST_LATEST_ITEMS');
                $item->columnheading . $i = MolajoTextHelper::_('JSTATUS');
                $item->columnheading . $i = MolajoTextHelper::_('LATEST_CREATED');
    ...
    public static function getTitle($parameters)
    {
        ...
        if ($catid) {
            ...
            if ($category) {
                ...
                $title = MolajoTextHelper::_('POPULAR_UNEXISTING');
            }
        }
        else
        {
