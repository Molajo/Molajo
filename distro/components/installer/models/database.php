...
class InstallerModelDatabase extends InstallerModelDisplay
{
    ...
    function initialise($options)
    {
        ...
        $jlang->load('joomla', MOLAJO_DISTRO, 'en-GB', true);
        // Load the selected language
        $jlang->load('joomla', MOLAJO_DISTRO, $options->language, true);

        // Ensure a database type was selected.
        if (empty($options->db_type)) {
            $this->setError(MolajoTextHelper::_('INSTL_DATABASE_INVALID_TYPE'));
            return false;
        }

        // Ensure that a valid hostname and user name were input.
        if (empty($options->db_host) || empty($options->db_user)) {
            $this->setError(MolajoTextHelper::_('INSTL_DATABASE_INVALID_DB_DETAILS'));
            return false;
        }

        // Ensure that a database name was input.
        if (empty($options->db_name)) {
            $this->setError(MolajoTextHelper::_('INSTL_DATABASE_EMPTY_NAME'));
            return false;
        }

        // Validate database table prefix.
        if (!preg_match('#^[a-zA-Z]+[a-zA-Z0-9_]*$#', $options->db_prefix)) {
            $this->setError(MolajoTextHelper::_('INSTL_DATABASE_PREFIX_INVALID_CHARS'));
            return false;
        }

        // Validate length of database table prefix.
        if (strlen($options->db_prefix) > 15) {
            $this->setError(MolajoTextHelper::_('INSTL_DATABASE_FIX_TOO_LONG'));
            return false;
        }

        // Validate length of database name.
        if (strlen($options->db_name) > 64) {
            $this->setError(MolajoTextHelper::_('INSTL_DATABASE_NAME_TOO_LONG'));
            return false;
        }
        ...
        if (empty($options->db_created)) {
            ...
            if (MolajoError::isError($db)) {
                $this->setError(MolajoTextHelper::sprintf('INSTL_DATABASE_COULD_NOT_CONNECT', (string)$db));
                return false;
            }

            // Check for database errors.
            if ($err = $db->getErrorNum()) {
                $this->setError(MolajoTextHelper::sprintf('INSTL_DATABASE_COULD_NOT_CONNECT', $db->getErrorNum()));
                return false;
            }
            ...
            if (!$db->select($options->db_name)) {
                ...
                if ($this->createDatabase($db, $options->db_name, $utfSupport)) {
                    ...
                    $this->setError(MolajoTextHelper::sprintf('INSTL_DATABASE_ERROR_CREATE', $options->db_name));
                    return false;
                }
            } else {
            ...
            if ($options->db_old == 'remove') {
                ...
                if (!$this->deleteDatabase($db, $options->db_name, $options->db_prefix)) {
                    $this->setError(MolajoTextHelper::_('INSTL_DATABASE_ERROR_DELETE'));
                    return false;
                }
            } else {
                // If the database isn't being deleted, back it up.
                if (!$this->backupDatabase($db, $options->db_name, $options->db_prefix)) {
                    $this->setError(MolajoTextHelper::_('INSTL_DATABASE_ERROR_BACKINGUP'));
                    return false;
                }
            }
            ...
            if (!$this->populateDatabase($db, $schema)) {
                $this->setError(MolajoTextHelper::sprintf('INSTL_ERROR_DB', $this->getError()));
                return false;
            }

            // Attempt to update the table #__schema.
            $files = JFolder::files(MOLAJO_PATH_ROOT . '/administrator/components/admin/sql/updates/mysql/', '\.sql$');
            if (empty($files)) {
                $this->setError(MolajoTextHelper::_('INSTL_ERROR_INITIALISE_SCHEMA'));
                return false;
            }
            $version = '';
            ...
            //					$this->setError(MolajoTextHelper::sprintf('INSTL_DATABASE_COULD_NOT_REFRESH_MANIFEST_CACHE', $extension->name));
            //					return false;
            //				}
            //			}
    ...
    function installSampleData($options)
    {
        ...
        // Attempt to import the database schema.
        if (!file_exists($data)) {
            $this->setError(MolajoTextHelper::sprintf('INSTL_DATABASE_FILE_DOES_NOT_EXIST', $data));
            return false;
        }
