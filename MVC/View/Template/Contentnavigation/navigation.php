...
class plgContentPagenavigation extends JPlugin
{
    ...
    public function onContentBeforeDisplay($context, &$row, &$params, $page = 0)
    {
        ...
        if ($params->get('show_item_navigation') && ($context == 'com_content.article') && ($view == 'article')) {
            ...
            $case_when1 .= ' ELSE ';
            $case_when1 .= $c_id . ' END as catalias';
            ...
            if ($row->prev) {
                $row->prev = JRoute::_(ContentHelperRoute::getArticleRoute($row->prev->alias, $row->prev->catalias));
            } else {
                $row->prev = '';
            }

            if ($row->next) {
                $row->next = JRoute::_(ContentHelperRoute::getArticleRoute($row->next->alias, $row->next->catalias));
            } else {
                $row->next = '';
            }
