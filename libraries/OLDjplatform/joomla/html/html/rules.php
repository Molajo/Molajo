...
abstract class JHtmlRules
{
	...
	public static function assetFormWidget($actions, $assetId = null, $parent = null, $control = 'jform[rules]', $idPrefix = 'jform_rules')
	{
		...
		$html[] = 		JHtml::_('tabs.panel',JText::_('MOLAJO_HTML_ACCESS_SUMMARY'), 'summary');
		$html[] = '			<p>'.JText::_('MOLAJO_HTML_ACCESS_SUMMARY_DESC').'</p>';
		$html[] = '			<table class="aclsummary-table" summary="'.JText::_('MOLAJO_HTML_ACCESS_SUMMARY_DESC').'">';
		$html[] = '			<caption>'.JText::_('MOLAJO_HTML_ACCESS_SUMMARY_DESC_CAPTION').'</caption>';
		$html[] = '			<tr>';
		$html[] = '				<th class="col1 hidelabeltxt">'.JText::_('MOLAJO_RULES_GROUPS').'</th>';
		...
		foreach ($actions as $action)
		{
			...
			$html[] = '			<caption>'.JText::_('MOLAJO_HTML_ACCESS_MODIFY_DESC_CAPTION_ACL').' '.$actionTitle.' '.JText::_('MOLAJO_HTML_ACCESS_MODIFY_DESC_CAPTION_TABLE').'</caption>';
			$html[] = '			<tr>';
			$html[] = '				<th class="col1 hidelabeltxt">'.JText::_('MOLAJO_RULES_GROUP').'</th>';
			$html[] = '				<th class="col2">'.JText::_('MOLAJO_RULES_INHERIT').'</th>';
			...
			foreach ($groups as $i => $group)
			{
				...
				$html[] = '				<td class="col3">';
				$html[] = '					<select id="'.$idPrefix.'_'.$action->name.'_'.$group->value.'" class="inputbox" size="1" name="'.$control.'['.$action->name.']['.$group->value.']" title="'.JText::sprintf('MOLAJO_RULES_SELECT_ALLOW_DENY_GROUP', $actionTitle, $group->text).'">';
				$html[] = '						<option value=""'.($selected === null ? ' selected="selected"' : '').'>'.JText::_('MOLAJO_RULES_INHERIT').'</option>';
				$html[] = '						<option value="1"'.($selected === true ? ' selected="selected"' : '').'>'.JText::_('MOLAJO_RULES_ALLOWED').'</option>';
				$html[] = '						<option value="0"'.($selected === false ? ' selected="selected"' : '').'>'.JText::_('MOLAJO_RULES_DENIED').'</option>';
				$html[] = '					</select>';
				$html[] = '				</td>';
		...
		$html[] = '		<li class="acl-allowed">'.JText::_('MOLAJO_RULES_ALLOWED').'</li>';
		$html[] = '		<li class="acl-denied">'.JText::_('MOLAJO_RULES_DENIED').'</li>';
		$html[] = '	</ul>';
		$html[] = '</div>';
	...
	{
		$base = JURI::root(true);
		$images['allow-l'] = '<label class="icon-16-allow" title="'.JText::_('MOLAJO_RULES_ALLOWED').'">'.JText::_('MOLAJO_RULES_ALLOWED').'</label>';
		$images['deny-l'] = '<label class="icon-16-deny" title="'.JText::_('MOLAJO_RULES_DENIED').'">'.JText::_('MOLAJO_RULES_DENIED').'</label>';
		$images['allow'] = '<a class="icon-16-allow" title="'.JText::_('MOLAJO_RULES_ALLOWED').'"> </a>';
		$images['deny'] = '<a class="icon-16-deny" title="'.JText::_('MOLAJO_RULES_DENIED').'"> </a>';
