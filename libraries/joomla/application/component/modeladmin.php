...
abstract class JModelAdmin extends JModelForm
{
	...
	protected function batchAccess($value, $pks)
	{
		...
		if (!$user->authorise('edit', $extension)) {
	...
	protected function batchCopy($value, $pks)
	{
		...
		$extension = JRequest::getCmd('option');
		$user = JFactory::getUser();
		if (!$user->authorise('create', $extension)) {
	...
	{
		$user = JFactory::getUser();
		return $user->authorise('delete', $this->option);
	}
	...
	{
		$user = JFactory::getUser();
		return $user->authorise('edit.state', $this->option);
	}
