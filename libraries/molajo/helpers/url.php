...
/**
 * @package     Molajo
 * @subpackage  Molajo Functions Links
...
class MolajoHelperURL {
    ...
    {
        if (substr($option_URL, 0, strlen(JPATH_ROOT)) == JPATH_ROOT) {
            return false;
        } elseif ( (strtolower(substr($option_URL, 0, 3)) == 'www') && (substr($option_URL, 3, strlen(JPATH_ROOT)) == JPATH_ROOT)) {
            return false;
        } else {
            return true;
        }
