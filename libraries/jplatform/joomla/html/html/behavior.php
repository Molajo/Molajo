...
abstract class JHtmlBehavior
{
	...
	public static function tooltip($selector='.hasTip', $params = array())
	{
		...
		if (!isset($opt['offset'])) {
			// Suppporting offsets parameter which was working in mootools 1.2 (Joomla1.5)
	...
	public static function uploader($id='file-upload', $params = array(), $upload_queue='upload-queue')
	{
		...
		if (!isset($uploaders)) {
			...
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_FILENAME');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_UPLOAD_COMPLETED');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_ERROR_OCCURRED');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_ALL_FILES');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_PROGRESS_OVERALL');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_CURRENT_TITLE');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_REMOVE');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_REMOVE_TITLE');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_CURRENT_FILE');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_CURRENT_PROGRESS');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_FILE_ERROR');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_FILE_SUCCESSFULLY_UPLOADED');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_DUPLICATE');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_SIZELIMITMIN');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_SIZELIMITMAX');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_FILELISTMAX');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_VALIDATION_ERROR_FILELISTSIZEMAX');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_ERROR_HTTPSTATUS');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_ERROR_SECURITYERROR');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_ERROR_IOERROR');
			JText::script('MOLAJO_HTML_BEHAVIOR_UPLOADER_ALL_FILES');
		}
		...
		$onFileSuccess = '\\function(file, response) {
			...
			if (json.get(\'status\') == \'1\') {
				...
				file.info.set(\'html\', \'<strong>\' + Joomla.JText._(\'MOLAJO_HTML_BEHAVIOR_UPLOADER_FILE_SUCCESSFULLY_UPLOADED\') + \'</strong>\');
			} else {
				file.element.addClass(\'file-failed\');
				file.info.set(\'html\', \'<strong>\' +
					Joomla.JText._(\'MOLAJO_HTML_BEHAVIOR_UPLOADER_ERROR_OCCURRED\', \'An Error Occurred\').substitute({ error: json.get(\'error\') }) + \'</strong>\');
			}
		}';
		...
		$opt['typeFilter']			= (isset($params['types'])) ? '\\'.$params['types'] : '\\{Joomla.JText._(\'MOLAJO_HTML_BEHAVIOR_UPLOADER_ALL_FILES\'): \'*.*\'}';
	...
	public static function calendar()
	{
		...
		$uncompressed	= JFactory::getConfig()->get('debug') ? '-uncompressed' : '';
		JHtml::_('stylesheet','system/calendar-jos.css', array(' title' => JText::_('MOLAJO_HTML_BEHAVIOR_GREEN') ,' media' => 'all'), true);
...
"\n\n" +
"'.JText::_('MOLAJO_HTML_BEHAVIOR_DATE_SELECTION',true).'" +
"'.JText::_('MOLAJO_HTML_BEHAVIOR_YEAR_SELECT',true).'" +
"'.JText::_('MOLAJO_HTML_BEHAVIOR_MONTH_SELECT',true).'" +
"'.JText::_('MOLAJO_HTML_BEHAVIOR_HOLD_MOUSE',true).'";
...
"- or click and drag for faster selection.";
		...
		Calendar._TT["PREV_YEAR"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_PREV_YEAR_HOLD_FOR_MENU',true).'";Calendar._TT["PREV_MONTH"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_PREV_MONTH_HOLD_FOR_MENU',true).'";	Calendar._TT["GO_TODAY"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_GO_TODAY',true).'";Calendar._TT["NEXT_MONTH"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_NEXT_MONTH_HOLD_FOR_MENU',true).'";Calendar._TT["NEXT_YEAR"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_NEXT_YEAR_HOLD_FOR_MENU',true).'";Calendar._TT["SEL_DATE"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_SELECT_DATE',true).'";Calendar._TT["DRAG_TO_MOVE"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_DRAG_TO_MOVE',true).'";Calendar._TT["PART_TODAY"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_TODAY',true).'";Calendar._TT["DAY_FIRST"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_DISPLAY_S_FIRST',true).'";Calendar._TT["WEEKEND"] = "0,6";Calendar._TT["CLOSE"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_CLOSE',true).'";Calendar._TT["TODAY"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_TODAY',true).'";Calendar._TT["TIME_PART"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_SHIFT_CLICK_OR_DRAG_TO_CHANGE_VALUE',true).'";Calendar._TT["DEF_DATE_FORMAT"] = "'.JText::_('%Y-%m-%d',true).'"; Calendar._TT["TT_DATE_FORMAT"] = "'.JText::_('%a, %b %e',true).'";Calendar._TT["WK"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_WK',true).'";Calendar._TT["TIME"] = "'.JText::_('MOLAJO_HTML_BEHAVIOR_TIME',true).'";';
			$jsscript = 1;
			return $return;
		}
		else {
