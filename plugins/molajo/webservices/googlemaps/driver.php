        ...
        for ( $i=0; $i < count($matches[0]); $i++ ) {
            ...
            for ($p=0; $p < count($options); $p++) {
                ...
                if ($x[0] == 'title') {
                    ...
                    $y = explode(',', $x[1]);
                    if (is_numeric($y[0])) { $latitude = $y[0]; } else { $latitude = 0; }
                    if (is_numeric($y[1])) { $longitude = $y[1]; } else { $longitude = 0; }

                } else if ($x[0] == 'zoom') {
                    if (((int) $x[1] > 0) && ((int) $x[1] < 22)) { $zoom = $x[1]; } else { $zoom = 0; }
                    
                } else if ($x[0] == 'size') {
                    $y = explode('x', $x[1]);
                    if (is_numeric($y[0])) { $width = $y[0]; } else { $width = 0; }
                    if (is_numeric($y[1])) { $height = $y[1]; } else { $height = 0; }
