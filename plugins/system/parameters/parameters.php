...
class plgSystemParameters extends MolajoPlugin
{
    ...
    function getParameterSetPath ($parameterSet)
    {
        ...
        $path = MOLAJO_PATH_SITE.'/templates/'.MolajoFactory::getApplication('site')->getTemplate().'/'.'parameters/'.$parameterSet.'.xml';
        if(is_file($path)) {
            return $path;
        }
        /** admin template parameters */
        $path = MOLAJO_PATH_ADMINISTRATOR.'/templates/'.MolajoFactory::getApplication('administrator')->getTemplate().'/'.'parameters/'.$parameterSet.'.xml';
        if(is_file($path)) {
            return $path;
        }
        /** component parameters */
        $path = MOLAJO_PATH_SITE.'/components/'.JRequest::getVar('component').'/'.'parameters/'.$parameterSet.'.xml';
        if(is_file($path)) {
            return $path;
        }
        /** administrator component */
        $path = MOLAJO_PATH_ADMINISTRATOR.'/components/'.JRequest::getVar('component').'/'.'parameters/'.$parameterSet.'.xml';
        if(is_file($path)) {
            return $path;
        }
        /** library */
        $path = MOLAJO_PATH_ROOT.MOLAJO_LAYOUTS_PARAMETERS.'/'.$parameterSet.'.xml';
        if(is_file($path)) {
            return $path;
        }
    }
    ...
    function getSiteLayouts ($option)
    {
        ...
        $path = MOLAJO_PATH_SITE.'/components/'.$option.'/views';
