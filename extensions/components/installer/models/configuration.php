...
class InstallerModelConfiguration extends MolajoModelDummy
{
    ...
    function _createConfiguration($config)
    {
        ...
        $registry->set('offline_message', MolajoTextHelper::_('INSTL_STD_OFFLINE_MSG'));
    ...
    function _createRootUser($config)
    {
        ...
        if (MolajoError::isError($db)) {
            $this->setError(MolajoTextHelper::sprintf('INSTL_ERROR_CONNECT_DB', (string)$db));
            return false;
        }

        // Check for database errors.
        if ($err = $db->getErrorNum()) {
            $this->setError(MolajoTextHelper::sprintf('INSTL_ERROR_CONNECT_DB', $db->getErrorNum()));
            return false;
        }
