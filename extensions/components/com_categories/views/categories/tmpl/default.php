...
?>
<form action="<?php echo MolajoRouteHelper::_('index.php?option=com_categories&view=categories');?>" method="post" name="adminForm" id="adminForm">
	...
	<fieldset id="filter-bar">
		...
		<div class="filter-select fltrt">
			...
			<select name="filter_published" class="inputbox" onchange="this.form.submit()">
				...
				<?php echo MolajoHTML::_('select.options', MolajoHTML::_('jgrid.publishedOptions'), 'value', 'text', $this->state->get('filter.published'), true);?>
			</select>
		...
		<thead>
			<tr>
				...
				<th width="10%">
					...
					<?php if ($saveOrder) :?>
						<?php echo MolajoHTML::_('grid.order',  $this->items, 'filesave.png', 'categories.saveorder'); ?>
					<?php endif; ?>
				</th>
				<th width="10%">
				...
				<th width="5%" class="nowrap">
					<?php echo MolajoHTML::_('grid.sort', 'JGRID_HEADING_LANGUAGE', 'language', $this->state->get('list.direction'), $this->state->get('list.ordering')); ?>
				</th>
				...
				<tr class="row<?php echo $i % 2; ?>">
					...
					<td>
						...
						<?php if ($item->checked_out) : ?>
							<?php echo MolajoHTML::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'categories.', $canCheckin); ?>
						<?php endif; ?>
						<?php if ($canEdit || $canEditOwn) : ?>
							<a href="<?php echo MolajoRouteHelper::_('index.php?option=com_categories&task=category.edit&id='.$item->id.'&extension='.$extension);?>">
					...
					<td class="center">
						<?php echo MolajoHTML::_('jgrid.published', $item->published, $i, 'categories.', $canChange);?>
					</td>
					<td class="order">
						<?php if ($canChange) : ?>
							<?php if ($saveOrder) : ?>
								<span><?php echo $this->pagination->orderUpIcon($i, isset($this->ordering[$item->parent_id][$orderkey - 1]), 'categories.orderup', 'MOLAJO_HTML_MOVE_UP', $ordering); ?></span>
								<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, isset($this->ordering[$item->parent_id][$orderkey + 1]), 'categories.orderdown', 'MOLAJO_HTML_MOVE_DOWN', $ordering); ?></span>
							<?php endif; ?>
