...
<form action="<?php echo MolajoRouteHelper::_('index.php?option=com_menus&view=items');?>" method="post" name="adminForm" id="adminForm">
	<fieldset id="filter-bar">
		...
		<div class="filter-select fltrt">
			...
			<select name="menu_id" class="inputbox" onchange="this.form.submit()">
				<?php echo MolajoHTML::_('select.options', MolajoHTML::_('menu.menus'), 'value', 'text', $this->state->get('filter.menu_id'));?>
			</select>
            ...
            <select name="filter_published" class="inputbox" onchange="this.form.submit()">
				...
				<?php echo MolajoHTML::_('select.options', MolajoHTML::_('jgrid.publishedOptions', array('archived' => false)), 'value', 'text', $this->state->get('filter.published'), true);?>
			</select>
	...
	<table class="adminlist">
		<thead>
			<tr>
				...
				<th width="5%">
					<?php echo MolajoHTML::_('grid.sort', 'JSTATUS', 'a.published', $listDirn, $listOrder); ?>
				</th>
				<th width="13%">
					<?php echo MolajoHTML::_('grid.sort', 'JGRID_HEADING_ORDERING', 'a.lft', $listDirn, $listOrder); ?>
					<?php if ($canOrder && $saveOrder) :?>
						<?php echo MolajoHTML::_('grid.order',  $this->items, 'filesave.png', 'items.saveorder'); ?>
					<?php endif; ?>
				</th>
				<th width="10%">
					<?php echo MolajoHTML::_('grid.sort',  'JGRID_HEADING_ACCESS', 'access_level', $listDirn, $listOrder); ?>
				</th>
				<th width="10%">
			...
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td>
					...
					<?php if ($item->checked_out) : ?>
						<?php echo MolajoHTML::_('jgrid.checkedout', $i, $item->editor, $item->checked_out_time, 'items.', $canCheckin); ?>
					<?php endif; ?>
					<?php if ($canEdit) : ?>
						<a href="<?php echo MolajoRouteHelper::_('index.php?option=com_menus&task=item.edit&id='.(int) $item->id);?>">
					...
					<p class="smallsub" title="<?php echo $this->escape($item->path);?>">
						...
						<?php elseif($item->type =='url' && $item->note) : ?>
							<?php echo MolajoText::sprintf('JGLOBAL_LIST_NOTE', $this->escape($item->note));?>
						<?php endif; ?></p>
				</td>
				<td class="center">
					<?php echo MolajoHTML::_('jgrid.published', $item->published, $i, 'items.', $canChange);?>
				</td>
				<td class="order">
					<?php if ($canChange) : ?>
						<?php if ($saveOrder) : ?>
							<span><?php echo $this->pagination->orderUpIcon($i, isset($this->ordering[$item->parent_id][$orderkey - 1]), 'items.orderup', 'MOLAJO_HTML_MOVE_UP', $ordering); ?></span>
							<span><?php echo $this->pagination->orderDownIcon($i, $this->pagination->total, isset($this->ordering[$item->parent_id][$orderkey + 1]), 'items.orderdown', 'MOLAJO_HTML_MOVE_DOWN', $ordering); ?></span>
						<?php endif; ?>
				...
				<td class="center">
					<?php if ($item->type == 'component') : ?>
						...
						<?php elseif ($canChange):?>
							<a href="<?php echo MolajoRouteHelper::_('index.php?option=com_menus&task=items.unsetDefault&cid[]='.$item->id.'&'.JUtility::getToken().'=1');?>">
