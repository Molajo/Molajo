...
class MenusControllerItem extends JControllerForm
{
	...
	public function add()
	{
		...
		if ($result) {
			...
			$this->setRedirect(MolajoRouteHelper::_('index.php?option=com_menus&view=item&menu_id='.$menuType.$this->getRedirectToItemAppend(), false));
		}
	...
	public function batch($model)
	{
		...
		// Preset the redirect
		$this->setRedirect(MolajoRouteHelper::_('index.php?option=com_menus&view=items'.$this->getRedirectToListAppend(), false));
	...
	public function save($key = null, $urlVar = null)
	{
		...
		if ($data === false) {
			...
			$this->setRedirect(MolajoRouteHelper::_('index.php?option='.$this->option.'&view='.$this->view_item.$this->getRedirectToItemAppend($recordId), false));
		...
		switch ($task) {
			...
			case 'save2new':
				...
				// Redirect back to the edit screen.
				$this->setRedirect(MolajoRouteHelper::_('index.php?option='.$this->option.'&view='.$this->view_item.$this->getRedirectToItemAppend(), false));
				break;
			...
			default:
				...
				$this->setRedirect(MolajoRouteHelper::_('index.php?option='.$this->option.'&view='.$this->view_list.$this->getRedirectToListAppend(), false));
				break;
		}
	}
