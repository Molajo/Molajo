...
class MolajoInstallationModelFilesystem extends JModel
{
	...
	{
		if (!is_writable(MOLAJO_SITE.'/tmp')) {
			return false;
		}
		if (!mkdir(MOLAJO_SITE.'/tmp/test', 0755)) {
			return false;
		}
		if (!copy(MOLAJO_SITE.'/tmp/index.html', MOLAJO_SITE.'tmp/test/index.html')) {
			return false;
		}
		if (!chmod(MOLAJO_SITE.'/tmp/test/index.html', 0777)) {
			return false;
		}
		if (!unlink(MOLAJO_SITE.'/tmp/test/index.html')) {
			return false;
		}
		if (!rmdir(MOLAJO_SITE.'/tmp/test')) {
			return false;
		}
	...
	static public function setFolderPermissions($folder, $options)
	{
		...
		if (!JPath::canChmod(JPath::clean(MOLAJO_SITE.'/'.$folder))) {
			$ftpFlag = true;
		}
		...
		if ($ftpFlag == true) {
			...
			$path = JPath::clean(MOLAJO_SITE.'/'.$folder);
	...
	function _chmod($path, $mode)
	{
		...
		// Initialise variables.
		$ftpFlag = true;
		$ftpRoot = $app->getConfiguration('ftp_root');

		// Do NOT use ftp if it is not enabled
		if ($app->getConfiguration(ftp_enable) != 1) {
			$ftpFlag = false;
		}

		if ($ftpFlag == true) {
			// Connect the FTP client
			$ftp = JFTP::getInstance($app->getConfiguration('ftp_host'), $app->getConfiguration('ftp_port'));
			$ftp->login($app->getConfiguration('ftp_user'), $app->getConfiguration('ftp_pass'));
