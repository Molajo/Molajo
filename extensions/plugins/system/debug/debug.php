 ...
 */
class plgSystemDebug extends MolajoPluginHelper
{
    /**
     * Constructor
     *
    ...
    function __destruct()
    {
        ...
        if (!empty($errors)) {
            echo '<h4>' . MolajoTextHelper::_('PLG_DEBUG_ERRORS') . '</h4><ol>';
            while ($error = JError::getError(true)) {
                echo '<li>' . $error->getMessage() . '<br /><h4>' . MolajoTextHelper::_('PLG_DEBUG_INFO') . '</h4><pre>' . print_r($error->get('info'), true) . '</pre><br /><h4>' . MolajoTextHelper::_('PLG_DEBUG_BACKTRACE') . '</h4>' . JError::renderBacktrace($error) . '</li>';
            }
            echo '</ol>';
        }

        if ($this->parameters->get('profile', 1)) {
            echo '<h4>' . MolajoTextHelper::_('PLG_DEBUG_PROFILE_INFORMATION') . '</h4>';
        ...
        if ($this->parameters->get('memory', 1)) {
            echo '<h4>' . MolajoTextHelper::_('PLG_DEBUG_MEMORY_USAGE') . '</h4>';
        ...
        if ($this->parameters->get('queries', 1)) {
            ...
            if ($log = $db->getLog()) {
                ...
                if ($this->parameters->get('query_types', 1)) {
                    ...
                    echo '<h4>' . MolajoTextHelper::sprintf('PLG_DEBUG_QUERY_TYPES_LOGGED', $totalQueryTypes) . '</h4>';

                    if ($totalSelectQueryTypes) {
                        echo '<h5>' . MolajoTextHelper::sprintf('PLG_DEBUG_SELECT_QUERIES') . '</h5>';
                        arsort($selectQueryTypeTicker);
                        echo '<ol>';

                        foreach ($selectQueryTypeTicker as $table => $occurrences)
                        {
                            echo '<li>' . MolajoTextHelper::sprintf('PLG_DEBUG_QUERY_TYPE_AND_OCCURRENCES', $table, $occurrences) . '</li>';
                        }
                    ...
                    if ($totalOtherQueryTypes) {
                        echo '<h5>' . MolajoTextHelper::sprintf('PLG_DEBUG_OTHER_QUERIES') . '</h5>';
        ...
        if (MolajoController::getApplication()->get('debug_language')) {
            ...
            if ($this->parameters->get('language_errorfiles', 1)) {
                ...
                if (count($errorfiles)) {
                    ...
                    echo '<pre>' . MolajoTextHelper::_('JNONE') . '</pre>';
                }
            }

            if ($this->parameters->get('language_files', 1)) {
                echo '<h4>' . MolajoTextHelper::_('PLG_DEBUG_LANGUAGE_FILES_LOADED') . '</h4>';
                echo '<ul>';
            ...
            if ($this->parameters->get('language_strings')) {
                ...
                echo '<h4>' . MolajoTextHelper::_('PLG_DEBUG_UNTRANSLATED_STRINGS') . '</h4>';
                echo '<pre>';
                ...
                if (count($orphans)) {
                    ...
                    {
                        echo "\n\n# " . ($file ? $file : MolajoTextHelper::_('PLG_DEBUG_UNKNOWN_FILE')) . "\n\n";
                        echo implode("\n", $keys);
                    }
                }
                else {
                    echo MolajoTextHelper::_('JNONE');
                }
                echo '</pre>';
            }
        }
