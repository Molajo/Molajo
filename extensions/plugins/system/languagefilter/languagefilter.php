...
class plgSystemLanguageFilter extends MolajoPlugin
{
	...
	public function __construct(&$subject, $config)
	{
        ...
        if (isset(self::$sefs[$sef])) {
            ...
            setcookie(MolajoUtility::getHash('language'), $lang_code, time() + 365 * 86400, $cookie_path, $cookie_domain);
            // set the request var
	...
	{
		$array = array();
		$lang_code = JRequest::getString(MolajoUtility::getHash('language'), null ,'cookie');
		if (!$lang_code) {
		...
		if (self::$mode_sef) {
			...
			if (!isset(self::$sefs[$sef])) {
				...
				{
					$app = MolajoFactory::getApplication();
					if ($app->getConfiguration('sef_rewrite')) {
