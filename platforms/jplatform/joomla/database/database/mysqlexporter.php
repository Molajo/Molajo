...
class JDatabaseExporterMySQL
{
        ...
        foreach ($this->from as $table)
        {
            ...
            foreach ($fields as $field)
            {
                $buffer[] = '   <field Field="' . $field->Field . '"' . ' Type="' . $field->Type . '"' . ' Null="' . $field->Null . '"' . ' Key="' .
                            $field->Key . '"' . (isset($field->Default) ? ' Default="' . $field->Default . '"'
                        : '') . ' Extra="' . $field->Extra . '"' .
                            ' />';
            }
    ...
    public function check()
    {
        ...
        if (!($this->db instanceof JDatabaseMySQL)) {
            throw new Exception('JPLATFORMS_ERROR_DATABASE_CONNECTOR_WRONG_TYPE');
        }

        // Check if the tables have been specified.
        if (empty($this->from)) {
            throw new Exception('JPLATFORMS_ERROR_NO_TABLES_SPECIFIED');
        }
    ...
    public function from($from)
    {
        ...
        {
            throw new Exception('JPLATFORMS_ERROR_INPUT_REQUIRES_STRING_OR_ARRAY');
        }
