    ...
    public function chmod($path, $mode)
    {
        ...
        if (FTP_NATIVE) {
            if (@ftp_site($this->_conn, 'CHMOD ' . $mode . ' ' . $path) === false) {
                if ($this->_OS != 'WIN') {
                    JError::raiseWarning('35', JText::_('JLIB_CLIENT_ERROR_JFTP_CHBAD_RESPONSE_NATIVE'));
                }
                return false;
            }
            return true;
        ...
        if (!$this->_putCmd('SITE CHMOD ' . $mode . ' ' . $path, array(200, 250))) {
            if ($this->_OS != 'WIN') {
                JError::raiseWarning('35', JText::sprintf('JLIB_CLIENT_ERROR_JFTP_CHBAD_RESPONSE', $this->_response, $path, $mode));
            }
            return false;
        }
        return true;
