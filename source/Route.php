...
Class Route
{
	...
	public function process()
	{
		 ...
		 */
		if ((int)Service::Registry()->get('DependencyInjection', 'catalog_id', 0) == 0) {
			Service::Registry()->set('Request', 'catalog_id', 0);
		} else {
			Service::Registry()->set('Request', 'catalog_id',
				(int)Service::Registry()->get('DependencyInjection', 'catalog_id', 0));
		}
		if (Service::Registry()->get('DependencyInjection', 'request_url', '') == '') {
			$path = PAGE_REQUEST;
		} else {
			$path = Service::Registry()->get('DependencyInjection', 'request_url', '');
		}
		...
		if ($continue == false) {
			Service::Debug()->set('Application::Route()->checkHome() Redirect to Real Home');
			return false;
		} else {
			Service::Debug()->set('Application::Route()->checkHome() No Redirect needed');
		}

		/**
		 *     See if Application is in Offline Mode
		 */
		if (Service::Registry()->get('Configuration', 'offline', 0) == 1) {
			Service::Error()->set(503);
			Service::Debug()->set('Application::Route() Direct to Offline Mode');
			return true;
		} else {
			Service::Debug()->set('Application::Route() Not in Offline Mode');
		}
		...
		if ($continue == false) {
			Service::Debug()->set('Application::Route()->getNonRoutableParameters() Failed');
			return false;
		} else {
			Service::Debug()->set('Application::Route()->getNonRoutableParameters() Successful');
		}
		...
		if ($continue == false) {
			Service::Debug()->set('Application::Route()->getCatalog() Failed');
			return false;
		} else {
			Service::Debug()->set('Application::Route()->getCatalog() Successful');

		}

		if (Service::Registry()->get('Catalog', 'catalog_type_id')
			== CATALOG_TYPE_MENU_ITEM_COMPONENT) {

			$continue = $this->getMenuitem();

			if ($continue == false) {
				Service::Debug()->set('Application::Route()->getMenuitem() Failed');
				return false;
			} else {
				Service::Debug()->set('Application::Route()->getMenuitem() Successful');
			}
		}
		...
		if (Service::Registry()->get('Request', 'status_found') === false) {
			Service::Error()->set(404);
			Service::Debug()->set('Application::Route() 404');
			return false;
		}
		 ...
		 */
		if ((int) Service::Registry()->get('Catalog', 'redirect_to_id', 0) == 0) {
		} else {
			Service::Response()->redirect(
				Application::Helper()->getURL('Catalog', Service::Registry()->get('Catalog', 'redirect_to_id', 0)), 301
			);
			Service::Debug()->set('Application::Route() Redirect');
			return false;
		}
		...
		if (Service::Registry()->get('Configuration', 'logon_requirement', 0) > 0
			...
			Service::Debug()->set('Application::Route() Redirect to Logon');
			return false;
		}
	...
	protected function checkHome($path = '')
	{
		...
		if (Service::Registry()->get('Request', 'request_url_query', '') == 'index.php'
			...
			Service::Redirect()->set('', 301);
			return false;
		}

		/** Home */
		if (Service::Registry()->get('Request', 'request_url_query', '') == ''
			&& (int)Service::Registry()->get('Request', 'catalog_id', 0) == 0
		) {
			Service::Registry()->set('Request', 'catalog_id',
	...
	{
		$path = Service::Registry()->get('Request', 'request_url_query');
		if ($path == '') {
			Service::Registry()->get('Request', 'non_routable_parameters', array());
			return true;
		}
		...
		foreach ($list->parameter as $item) {
			...
			$value = Service::Request()->get('request')->get($key);
		...
		Service::Registry()->set('Request', 'request_url_query', $path);
		Service::Registry()->set('Request', 'non_routable_parameters', $use);
	...
	{
		try {
			$value = Service::Filter()->filter($value, $dataType, $null, $default);
	...
	protected function getCatalog()
	{
		...
		$row = Application::Helper()
			->get('Catalog',
				(int)Service::Registry()->get('Request', 'catalog_id'),
		...
		/** Catalog Registry */
		Service::Registry()->set('Catalog', 'id', (int)$row->id);
		Service::Registry()->set('Catalog', 'redirect_to_id', (int)$row->redirect_to_id);
		Service::Registry()->set('Catalog', 'catalog_type_id', (int)$row->catalog_type_id);
		Service::Registry()->set('Catalog', 'source_id', (int)$row->source_id);
		Service::Registry()->set('Catalog', 'view_group_id', (int)$row->view_group_id);
		Service::Registry()->set('Catalog', 'primary_category_id', (int)$row->primary_category_id);
		Service::Registry()->set('Catalog', 'sef_request', $row->sef_request);
		Service::Registry()->set('Catalog', 'request', $row->request);
//		Service::Registry()->set('Catalog', 'request_option', $row->request_option);
//		Service::Registry()->set('Catalog', 'request_model', $row->request_model);
		Service::Registry()->set('Catalog', 'source_table', $row->source_table);

		/** home */
		if ((int)Service::Registry()->get('Catalog', 'catalog_id', 0)
	...
	protected function getMenuitem()
	{
		...
		$row = Application::Helper()
			->get('Menuitem',
				(int)Service::Registry()->get('Catalog', 'source_id')
			);
...
echo '</pre>';
		die;
		/** Catalog Registry */
		Service::Registry()->set('Menuitem', 'id', (int)$row->id);
		Service::Registry()->set('Menuitem', 'redirect_to_id', (int)$row->redirect_to_id);
		Service::Registry()->set('Menuitem', 'catalog_type_id', (int)$row->catalog_type_id);
		Service::Registry()->set('Menuitem', 'source_id', (int)$row->source_id);
		Service::Registry()->set('Menuitem', 'view_group_id', (int)$row->view_group_id);
		Service::Registry()->set('Menuitem', 'primary_category_id', (int)$row->primary_category_id);
		Service::Registry()->set('Menuitem', 'sef_request', $row->sef_request);
		Service::Registry()->set('Menuitem', 'request', $row->request);
//		Service::Registry()->set('Menuitem', 'request_option', $row->request_option);
//		Service::Registry()->set('Menuitem', 'request_model', $row->request_model);
		Service::Registry()->set('Menuitem', 'source_table', $row->source_table);

		/** home */
		if ((int)Service::Registry()->get('Menuitem', 'catalog_id', 0)
			== Service::Registry()->get('Configuration', 'home_catalog_id', null)
		) {
			Service::Registry()->set('Request', 'request_url_home', true);
		} else {
		...
		if (Service::Registry()->get('Request', 'request_catalog_type_id') == CATALOG_TYPE_MENU_ITEM_COMPONENT) {
