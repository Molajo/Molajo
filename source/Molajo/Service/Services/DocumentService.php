...
Class DocumentService
{
	...
	{
		if (Service::Filesystem()->folderExists($file_path . '/css')) {
		} else {
			return;
		}

		$files = Service::Filesystem()->folderFiles($file_path . '/css', '\.css$', false, false);
	...
	public function add_js_folder($file_path, $url_path, $priority = 500, $defer = 0)
	{
		...
		if (Service::Filesystem()->folderExists($file_path . $extra)) {
		} else {
			return;
		}

		$files = Service::Filesystem()->folderFiles($file_path . $extra, '\.js$', false, false);
