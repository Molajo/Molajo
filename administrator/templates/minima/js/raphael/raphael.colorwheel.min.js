...
u[p].drag=function(a,b,c){this._drag={};var d=this.mousedown(function(g){(g.originalEvent?g.originalEvent:g).preventDefault();this._drag.x=g.applicationX;this._drag.y=g.applicationY;this._drag.id=g.identifier;b&&b.call(this,g.applicationX,g.applicationY);Raphael.mousemove(f).mouseup(e)});function f(g){var h=g.applicationX,i=g.applicationY;if(Ba)for(var j=g.touches.length,l;j--;){l=g.touches[j];if(l.identifier==d._drag.id){h=l.applicationX;i=l.applicationY;(g.originalEvent?g.originalEvent:g).preventDefault();break}}else g.preventDefault();
