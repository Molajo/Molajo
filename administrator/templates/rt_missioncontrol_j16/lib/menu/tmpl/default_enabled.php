...
if ($user->authorise('manage', 'com_users'))
{
	...
	$createGrp	= $createUser &&  $user->authorise('admin', 'com_users');
...
if ($user->authorise('manage', 'com_content'))
{
	...
	$createContent =  $shownew && $user->authorise('create', 'com_content');
	$menu->addChild(
...
{
	$menu->addChild(new JMenuNode(JText::_('MOD_MENU_EXTEND')), true);
