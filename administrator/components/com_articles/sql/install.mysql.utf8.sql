...
(1, 0, 0, 1, 'com_articles', 'com_articles', '{"admin":{"7":1},"manage":{"6":1},"create":{"3":1},"delete":[],"edit":{"4":1},"edit.state":{"5":1},"edit.own":[]}');

/* Update RGT for Component Asset Record */
SET @max_rgt = (SELECT MAX(rgt)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET rgt = @max_rgt WHERE `name` = 'com_articles';

/* Update LFT for Component Asset Record */
SET @max_lft = (SELECT MAX(lft)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET lft = @max_lft WHERE `name` = 'com_articles';
...
INSERT INTO `#__categories` (`level`, `parent_id`, `path`, `extension`, `title`, `alias`, `note`, `description`, `published`, `checked_out`, `checked_out_time`, `access`, `params`, `metadesc`, `metakey`, `metadata`, `created_user_id`, `created_time`, `modified_user_id`, `modified_time`, `hits`, `language`) VALUES
    (1, 1, 'uncategorised', 'com_articles', 'Uncategorised', 'uncategorised', '', '', 1, 0, '0000-00-00 00:00:00', 1, '{"target":"","image":""}', '', '', '{"page_title":"","author":"","robots":""}', 42, '2010-06-28 13:26:37', 0, '0000-00-00 00:00:00', 0, '*');

/* Update RGT for Category Record */
SET @max_rgt = (SELECT MAX(rgt)+1 FROM `#__categories` WHERE `id` <> 1);
UPDATE `#__categories` SET rgt = @max_rgt WHERE `extension` = 'com_articles';

/* Update LFT for Category Record */
SET @max_lft = (SELECT MAX(lft)+1 FROM `#__categories` WHERE `id` <> 1);
UPDATE `#__categories` SET lft = @max_lft WHERE `extension` = 'com_articles';

/** 3. ASSET RECORD FOR CATEGORY **/

/* Create Category Asset Record */
INSERT INTO `#__assets` (`level`, `name`, `title`, `rules`)
    SELECT 2 as `level`, CONCAT('com_articles.category', '.', id) as `name`, title as `title`, '{"create":[],"delete":[],"edit":[],"edit.state":[],"edit.own":[]}' as `rules`
     FROM `#__categories`
     WHERE extension = 'com_articles';

/* Update Category Record for Asset ID */
SET @asset_id = (SELECT id FROM `#__assets` WHERE `name` like 'com_articles.category.%');
UPDATE `#__categories` SET asset_id = @asset_id WHERE `extension` = 'com_articles';

/* Update Parent ID for Component Asset Record */
SET @parent_id = (SELECT id FROM `#__assets` WHERE `name` = 'com_articles');
UPDATE `#__assets` SET parent_id = @parent_id WHERE `name` like 'com_articles.category.%';

/* Update RGT for Component Asset Record */
SET @max_rgt = (SELECT MAX(rgt)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET rgt = @max_rgt WHERE `name` like 'com_articles.category.%';

/* Update LFT for Component Asset Record */
SET @max_lft = (SELECT MAX(lft)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET lft = @max_lft WHERE `name` like 'com_articles.category.%';
...
/* Update Category Record for Asset ID */
SET @catid = (SELECT id FROM `#__categories` WHERE `extension` = 'com_articles');
UPDATE `#__articles` SET catid = @catid;

/** 5. ASSET RECORD FOR CONTENT **/

/* Create Content Asset Record */
INSERT INTO `#__assets` (`level`, `name`, `title`, `rules`)
    SELECT 3 as `level`, CONCAT('com_articles.article', '.', id) as `name`, title as `title`, '{"create":[],"delete":[],"edit":[],"edit.state":[],"edit.own":[]}' as `rules`
     FROM `#__articles`;

/* Update Category Record for Asset ID */
SET @asset_id = (SELECT id FROM `#__assets` WHERE `name` like 'com_articles.article.%');
UPDATE `#__articles` SET asset_id = @asset_id;

/* Update Parent ID for Component Asset Record */
SET @parent_id = (SELECT id FROM `#__assets` WHERE `name` = 'com_articles');
UPDATE `#__assets` SET parent_id = @parent_id WHERE `name` like 'com_articles.article.%';

/* Update RGT for Component Asset Record */
SET @max_rgt = (SELECT MAX(rgt)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET rgt = @max_rgt WHERE `name` like 'com_articles.article.%';

/* Update LFT for Component Asset Record */
SET @max_lft = (SELECT MAX(lft)+1 FROM `#__assets` WHERE `id` <> 1);
UPDATE `#__assets` SET lft = @max_lft WHERE `name` like 'com_articles.article.%';
