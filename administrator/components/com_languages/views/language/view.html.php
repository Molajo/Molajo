...
class LanguagesViewLanguage extends JView
{
	...
	protected function addToolbar()
	{
		...
		if ($isNew && $canDo->get('core.create')) {
			MolajoToolbarHelper::save('language.save');
		}

		//If an existing item, allow to Apply and Save.
		if (!$isNew && $canDo->get('core.edit')) {
			MolajoToolbarHelper::apply('language.apply');
			MolajoToolbarHelper::save('language.save');
		}

		// If an existing item, can save to a copy only if we have create rights.
		if ($canDo->get('core.create')) {
			MolajoToolbarHelper::save2new('language.save2new');
		}

		if ($isNew) {
			MolajoToolbarHelper::cancel('language.cancel');
		}
		else {
			MolajoToolbarHelper::cancel('language.cancel', 'JTOOLBAR_CLOSE');
		}

		MolajoToolbarHelper::divider();
		MolajoToolbarHelper::help('JHELP_EXTENSIONS_LANGUAGE_MANAGER_EDIT');
	}
}
