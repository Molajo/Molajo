...
$chm = $user->authorise('admin', 'com_checkin');
$cam = $user->authorise('manage', 'com_cache');
...
if ($user->authorise('manage', 'com_users'))
{
	...
	$createUser = $shownew && $user->authorise('create', 'com_users');
	$createGrp	= $user->authorise('admin', 'com_users');
...
if ($user->authorise('manage', 'com_menus'))
{
	...
	$createMenu = $shownew && $user->authorise('create', 'com_menus');
...
// Content Submenu
//
if ($user->authorise('manage', 'com_articles'))
{
	$menu->addChild(
	new JMenuNode(JText::_('MOD_MENU_COM_CONTENT'), '#'), true
	);
	$createContent =  $shownew && $user->authorise('create', 'com_articles');
	$menu->addChild(
